...
class HoprdAPI:
    ...
    async def __call(self, method: HTTPMethod, endpoint: str, data: ApiRequestObject = None, use_prefix: bool = True):
        try:
            headers = {"Content-Type": "application/json"}
            async with aiohttp.ClientSession(headers=self.headers) as s:
                async with getattr(s, method.value)(
                    url=f"{self.host}{self.prefix if use_prefix else '/'}{endpoint}",
    ...
    async def __call_api(
        ...
        try:
            return await asyncio.wait_for(
                asyncio.create_task(self.__call(method, endpoint, data, use_prefix)),
                timeout=timeout,
            )
    ...
    async def metrics(self):
        _, response = await self.__call_api(HTTPMethod.GET, "metrics")
        return response
