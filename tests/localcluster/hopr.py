...
class HoprdAPI:
    ...
    def __call_api(self, obj: Callable[..., object], method: str, *args, **kwargs) -> tuple[bool, Optional[object]]:
        try:
            ...
            log.error(
                f"OSError calling {api_callback.__qualname__} with kwargs: {kwargs}, args: {args}:")
            return (False, None)
        except MaxRetryError:
            log.error(
                f"MaxRetryError calling {api_callback.__qualname__} with kwargs: {kwargs}, args: {args}")
            return (False, None)
    ...
    async def channels_fund_channel(self, channel_id: str, amount: str):
        ...
        status, _ = self.__call_api(
            ChannelsApi, "fund_channel", body, channel_id)
        return status
    ...
    async def channel_redeem_tickets(self, channel_id: str):
        ...
        """
        status, _ = self.__call_api(
            ChannelsApi, "redeem_tickets_in_channel", channel_id)
        return status
    ...
    async def channels_aggregate_tickets(self, channel_id: str):
        ...
        """
        status, _ = self.__call_api(
            ChannelsApi, "aggregate_tickets_in_channel", channel_id)
        return status
    ...
    async def channel_get_tickets(self, channel_id: str):
        ...
        """
        status, response = self.__call_api(
            ChannelsApi, "show_channel_tickets", channel_id)
    ...
    async def peers(self, params: list or str = "peer_id", status: str = "connected"):
        ...
        if is_ok:
            ...
            for peer in getattr(response, status):
                output_list.append({param: getattr(peer, param)
                                   for param in params})
    ...
    async def send_message(self, destination: str, message: str, hops: list[str], tag: int = MESSAGE_TAG) -> bool:
        ...
        :return: bool
        """
        body = SendMessageBodyRequest(
            body=message, hops=None, path=hops, destination=destination, tag=tag)
    ...
    async def session_client(
        ...
        if capabilities is None:
            body = SessionClientRequest(
                destination=destination, path=path, target=actual_target, listen_host=listen_on)
        else:
            body = SessionClientRequest(
        ...
        _, response = self.__call_api(
            SessionApi, "create_client", body=body, protocol=protocol)
    ...
    async def session_list_clients(self, protocol: str):
        ...
        """
        _, response = self.__call_api(
            SessionApi, "list_clients", protocol=protocol)
        return response
    ...
    async def session_close_client(self, protocol: str, bound_port: int, bound_ip: str = "127.0.0.1"):
        ...
        """
        body = SessionCloseClientRequest(
            listening_ip=bound_ip, port=bound_port)

        status, _ = self.__call_api(
            SessionApi, "close_client", body=body, protocol=protocol)
        return status
