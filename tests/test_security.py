...
def test_hoprd_apis_should_be_available(setup_7_nodes):
    ...
    wait(lambda: check_socket(LOCALHOST, secure_api_port),
         timeout_seconds=1,
         waiting_for=f"API port {secure_api_port} to become available")

    insecure_api_port = setup_7_nodes['2']['api_port']
    wait(lambda: check_socket(LOCALHOST, insecure_api_port),
         timeout_seconds=1,
...
def test_hoprd_websocket_api_should_accept_connection_with_token_as_query_param(setup_7_nodes):
    ws = websocket.WebSocket()
...
def test_hoprd_websocket_noauth_api_should_accept_connection_without_a_token(setup_7_nodes):
    ws = websocket.WebSocket()
...
def test_hoprd_websocket_noauth_api_should_accept_connection_with_an_invalid_token(setup_7_nodes):
    ws = websocket.WebSocket()
    ws.connect(f"ws://{LOCALHOST}:{setup_7_nodes['2']['api_port']}/",
...
def test_hoprd_websocket_api_should_reject_connection_on_invalid_path(setup_7_nodes):
    ws = websocket.WebSocket()
    try:
        ws.connect(f"ws://{LOCALHOST}:{setup_7_nodes['1']['api_port']}/inVaLiD_paTh",
                   cookie=f"X-Auth-Token={DEFAULT_API_TOKEN}")
