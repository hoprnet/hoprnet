...
- `tracing`: Enable Jaeger OTLP ingestion for hoprd telemetry. Set `HOPRD_USE_OPENTELEMETRY=true`, configure `HOPRD_OTEL_SIGNALS` (for example `traces` or `traces,logs,metrics`), set `HOPRD_OTLP_ENDPOINT`, and configure `HOPRD_METRIC_EXPORT_INTERVAL` as `default_ms,prefix=ms` when needed (for example `15000,hopr_session=1000`).
