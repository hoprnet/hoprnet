...
/// In-memory peer store for network multiaddresses.
pub mod peer_store;
...
pub struct HoprNetwork {
    ...
    /// Shared liveness registry. Entries are cleared by the swarm event loop
    /// on `ConnectionClosed` / `OutgoingConnectionError` so that wrapped
    /// substreams for that peer self-error on their next poll.
...
impl NetworkStreamControl for HoprNetwork {
    ...
    ) -> Result<impl futures::Stream<Item = (PeerId, impl AsyncRead + AsyncWrite + Send)> + Send, impl std::error::Error>
    {
        ...
        self.control.accept(self.protocol).map(|stream| {
            stream.map(move |(peer, inner)| {
                let flag = liveness.liveness(&peer);
    ...
    async fn open(mut self, peer: PeerId) -> Result<impl AsyncRead + AsyncWrite + Send, impl std::error::Error> {
        let flag = self.liveness.liveness(&peer);
        self.control
            .open_stream(peer, self.protocol)
            .await
            .map(|inner| LivenessStream::new(inner, flag))
    }
}
