...
/// the loop calls [`LivenessRegistry::mark_peer_disconnected`], which removes
...
impl LivenessRegistry {
    ...
    pub(crate) fn peer_liveness(&self, peer: &PeerId) -> Arc<AtomicBool> {
        self.0
            .entry(*peer)
    ...
    ///
    /// A subsequent `open()` call mints a fresh `true` flag via [`Self::peer_liveness`].
    /// Any [`LivenessStream`] that still holds the old `Arc` will error on its next poll.
    pub(crate) fn mark_peer_disconnected(&self, peer: &PeerId) {
...
mod tests {
    ...
    fn peer_liveness_should_create_alive_flag_for_new_peer() {
        ...
        let flag = registry.peer_liveness(&peer);
    ...
    fn peer_liveness_should_return_same_flag_for_same_peer() {
        ...
        let flag1 = registry.peer_liveness(&peer);
        let flag2 = registry.peer_liveness(&peer);
    ...
    fn mark_peer_disconnected_should_clear_flag_and_remove_entry() {
        ...
        registry.mark_peer_disconnected(&peer);
    ...
    fn subsequent_peer_liveness_after_disconnect_should_return_fresh_alive_flag() {
        ...
        let old_flag = registry.peer_liveness(&peer);
        registry.mark_peer_disconnected(&peer);

        let new_flag = registry.peer_liveness(&peer);
