...
pub fn cluster_fixture(#[default(vec![TestNodeConfig::default(); 3])] configs: Vec<TestNodeConfig>) -> ClusterGuard {
    cluster_fixture_inner(configs, build_blokli_client(), EchoMode::Echo)
}

/// What the per-node [`EchoServer`] does with the bytes it receives.
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
enum EchoMode {
    /// Tally received bytes *and* write them back — needed by bidirectional tests.
    Echo,
    /// Only tally received bytes. Used by the one-way throughput harness, which never
    /// supplies return-path SURBs (see [`EchoServer::with_counter`]).
    CountOnly,
}
            ...
            std::thread::spawn(move || {
                ...
                let result = runtime.block_on(async {
                    ...
                    let instance = crate::testing::wiring::build_full_with_chain(
                        ...
                        connector.clone(),
                        match echo_mode {
                            EchoMode::Echo => EchoServer::with_counter_and_echo(echo_counter),
                            EchoMode::CountOnly => EchoServer::with_counter(echo_counter),
                        },
                    )
                    .await?;
