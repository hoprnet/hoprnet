    ...
    Net: NetworkView + NetworkStreamControl + Send + Sync + Clone + 'static,
{
    ...
    //
    // info!(peer_id = %self.me_peer_id(), address = %self.me_onchain(), version = constants::APP_VERSION, "Node
    // information");
    //
    ...
    // &self
    // .chain_api
    // .stream_channels(ChannelSelector::default().with_destination(self.chain_id.node_address))
    // .map_err(HoprLibError::chain)?
    // .collect::<Vec<_>>()
    // .await,
    // )
