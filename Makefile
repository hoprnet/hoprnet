...
run-local: ## run HOPRd from local repo
	env NODE_OPTIONS="--experimental-wasm-modules" NODE_ENV=development DEBUG="hopr*" node \
		...
		--network-name anvil-localhost --announce \
...
environment_type != jq '.networks."$(network_name)".environment_type // empty' packages/ethereum/contracts/contracts-addresses.json
