...
export function privKeyToPeerId(privKey: Uint8Array | string): Promise<PeerId> {
  ...
  const secp256k1PrivKey = new libp2p_crypto.supportedKeys.secp256k1.Secp256k1PrivateKey(Buffer.from(privKey), secp256k1.publicKeyCreate(Buffer.from(privKey)))
