...
describe('test utils', function() {
  ...
  it('should sign and verify messages', async function() {
    ...
    const signature = await sign(message, privKey, pubKey)
    assert(await verify(message, signature, pubKey), `check that signature is verifiable`)

    message[0] ^= 0xff
    assert(!(await verify(message, signature, pubKey)), `check that manipulated message is not verifiable`)
  })
})
