...
rec {
  ...
  mkLocalBuilder = { useRustNightly ? false }:
    import ../rust-builder.nix {
      inherit nixpkgs rust-overlay crane foundry solc localSystem useRustNightly;
    };
  ...
  mkAarch64LinuxBuilder = {}:
    import ../rust-builder.nix {
      inherit nixpkgs rust-overlay crane foundry solc localSystem;
      crossSystem = (import nixpkgs { inherit localSystem; }).lib.systems.examples.aarch64-multiplatform-musl;
      isCross = true;
      isStatic = true;
    };
