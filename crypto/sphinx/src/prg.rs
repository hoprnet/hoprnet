...
pub trait PRG: for<'a> From<&'a SecretKey> {
    ...
    fn digest(&mut self, from: usize, to: usize) -> Box<[u8]>;
}
...
impl From<&SecretKey> for AesCtrPRG {
    fn from(value: &SecretKey) -> Self {
        let mut ret = PRGParameters::default();
        generate_key_iv(value, HASH_KEY_PRG.as_bytes(), &mut ret.key, &mut ret.iv, true);
        Self::from_parameters(ret)
    }
}
...
mod tests {
    ...
    fn test_prg_single_block() {
        let out = AesCtrPRG::from_parameters(PRGParameters {
            key: [0u8; 16],
            iv: [0u8; 12],
        })
        .digest(5, 10);
