...
use hopr_api::types::{crypto::prelude::*, primitive::prelude::*};
...
pub fn generate_proof_of_relay(secrets: &[SharedSecret]) -> Result<(Vec<ProofOfRelayString>, ProofOfRelayValues)> {
    ...
    for i in 0..secrets.len() {
        ...
        let next_ticket_challenge = if let Some(next_secret) = secrets.get(i + 1) {
            ...
            EthereumChallenge(hopr_api::types::crypto_random::random_bytes::<{ Address::SIZE }>().into())
        };
...
mod tests {
    ...
    impl ProofOfRelayValues {
        fn create(
            ...
            chain_length: u8,
        ) -> hopr_api::types::crypto::errors::Result<(Self, HalfKey)> {
    ...
    impl ProofOfRelayString {
        ...
        ) -> hopr_api::types::crypto::errors::Result<Vec<ProofOfRelayString>> {
            (1..secrets.len())
