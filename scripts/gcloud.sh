...
gcloud_vm_name() {
  ...
  echo "hopr-${VERSION_MAJ_MIN//./-}-$RELEASE_NAME-$1"
}
...
gcloud_get_address() {
  local ip
  ip=$(gcloud compute addresses describe "$1" "$REGION" 2>&1)

  # Google does not return an appropriate exit code :(
  if grep -q 'ERROR' "$ip"; then
...
gcloud_get_logs() {
  ...
  local id
  id=$(gcloud compute ssh "$ZONE" "$1" --command "docker ps -q --filter ancestor='$2' | xargs docker inspect --format='{{.Id}}'" | grep -v 'warning')
