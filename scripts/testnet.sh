...
get_network() {
  local network="${1}"
  jq -r ".networks.\"${network}\".chain" "${mydir}/../hopr/hopr-lib/data/protocol-config.json"
}
...
get_rpc() {
  ...
  jq -r ".chains.\"${chain}\".default_provider" "${mydir}/../hopr/hopr-lib/data/protocol-config.json" | envsubst
}
