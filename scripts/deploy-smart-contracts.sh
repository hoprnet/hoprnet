...
for git_ref in $(cat "${release_config}" | jq -r "to_entries[] | .value.git_ref" | uniq); do
  if [[ "${branch}" =~ ${git_ref} ]]; then
    for network_name in $(cat "${release_config}" | jq -r "to_entries[] | select(.value.git_ref==\"${git_ref}\") | .value.network_name"); do
      declare chain=$(cat "${protocol_config}" | jq -r ".networks.\"${network_name}\".chain")
      declare environment_type=$(cat "${deployments_summary}" | jq -r ".networks.\"${network_name}\".environment_type")

      log "deploying for network ${network_name} on chain ${chain} of type ${environment_type}"

      make -C "${mydir}/../packages/ethereum/contracts/" anvil-deploy-contracts network-name="${network_name}" environment-type="${environment_type}"

      # update the deployed files in protocol-config
      update_protocol_config_addresses "${protocol_config}" "${deployments_summary}" "${network_name}" "${network_name}"
    done
  fi
done
