...
for git_ref in $(cat "${release_config}" | jq -r "to_entries[] | .value.git_ref" | uniq); do
  if [[ "${branch}" =~ ${git_ref} ]]; then
    for network_id in $(cat "${release_config}" | jq -r "to_entries[] | select(.value.git_ref==\"${git_ref}\") | .value.network_id"); do
      declare chain=$(cat "${protocol_config}" | jq -r ".networks.\"${network_id}\".chain")
      declare environment_type=$(cat "${deployments_summary}" | jq -r ".networks.\"${network_id}\".environment_type")

      log "deploying for network ${network_id} on chain ${chain} of type ${environment_type}"

      make -C "${mydir}/../packages/ethereum/contracts/" anvil-deploy-contracts network-id="${network_id}" environment-type="${environment_type}"

      # update the deployed files in protocol-config
      update_protocol_config_addresses "${protocol_config}" "${deployments_summary}" "${network_id}" "${network_id}"
    done
  fi
done
