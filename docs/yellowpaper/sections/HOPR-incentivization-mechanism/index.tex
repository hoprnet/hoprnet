...
\begin{itemize}
    \item \textbf{Interactively:} This is done by dispatching an on-chain call to \textit{close()}. This changes the on-chain state to $closed_A$ or $closed_B$ and allows the redemption of previously unredeemed tickets. 
    ...
    \item \textbf{Immediately witn a signature:} This can be done using a single call to \textit{closewithSignature()} by party A and the other party B can give its consent to close the channel by calling \textit{agreeB()}.
\end{itemize}
