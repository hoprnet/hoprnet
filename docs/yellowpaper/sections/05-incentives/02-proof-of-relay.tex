...
% \begin{itemize}
%       \item The creator of the packet creates a secret sharing between itself and the first relayer as well as for each relayer $n_i$ and their corresponding next downstream node $n_{i+1}$
%       \item Having both key shares allows the node to reconstruct a value $response$.
%       \item $response$ is the solution necessary to solve a $challenge$.
%       \item
% \end{itemize}

% First of all, the creator of the packet samples challenges and their responses for each relayer and its corresponding next downstream node, including the first relayer. Whenever a node issues a ticket (see section \ref{sec:tickets}), it embeds the corresponding ch

% secret sharings for each relayer and its next downstream node. These secret sharings

% Acknowledgement structure + utilized signatures
