...
The \nameref{sec:sphinx} guarantees that packets can only be processed in the order that has been chosen by the creator. As a result, the next downstream node can either decode the packet or end up with some random bitstring. Hence it is the next downstream node who is able to verify the correctness of the previously applied tranformation which it confirms by sending an acknowledgement.
...
% Acknowledgements are sent out of order because otherwise introducing observable patterns
