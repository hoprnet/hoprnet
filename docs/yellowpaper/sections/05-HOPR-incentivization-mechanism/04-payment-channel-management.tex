...
\begin{tikzpicture}[every text node part/.style={align=center}]
    ...
    \draw[->] (8+\nodeWidth/2,\nodeHeight) to [bend right] (4+\nodeWidth,2+\nodeHeight/2);
    \draw[->] (\nodeWidth/2,0) to [bend right] (4,-2+\nodeHeight/2);
    \draw[->] (4+\nodeWidth,-2+\nodeHeight/2) to [bend right] (8+\nodeWidth/2,0);

    \draw[->] (4+\nodeWidth/2,-2+\nodeHeight) -- (4+\nodeWidth/2,2) node[midway,above=30pt,right=4pt] {\tiny{Initiate closure}\\\tiny{(if not committed)}};
    ...
    % \draw[name=open,rounded corners=6mm]  (0,0) rectangle (3,3) node[midway] {Open};
    % \draw[name=waitingForCommitment,rounded corners=6mm]  (0,0) rectangle (3,3) node[midway] {WaitingForCommitment};
\end{tikzpicture}
