...
The destination address of the address must now commit in order for the channel between both parties to be open. 
...
\begin{scope}[very thick,->]
    ...
    \draw [->,draw](closed) to [left] node {\naput{fund()}} (open);
    \draw [->,draw](commitment) to [bend left] node {\naput{fund()}} (open);
