...
$h:G^*\times G^*\rightarrow\mathbb{Z}^*_q$ where $\mathbb{Z}^*_q$ is the field of non-identity elements of $\mathbb{Z}_q$ (field of integers).
    ...
    and
    $$\beta_i=n_{i+1}\|\gamma_{i+1}\|\beta_{{i+1}_{[ \,0....(2r-1)\kappa-1\,] }}\oplus PRNG(s_{i})_{[ \,0....(2r+1)\kappa-1\,]} $$ for $0\le i < v-1$ and $\phi_i=\{ \phi_{i-1}\|0_{2\kappa}\}\oplus PRNG(s_{i-1})_{[ \,(2(r-i)+3)\kappa..(2r+3)\kappa-1\,]}$ and $\phi_0=\epsilon$ is an empty string
...
After that $(B)$ extracts the public key of $(C)$, it deletes the routing information from the packet. Afterwards, it fills the empty space with its own blinding which is different from the one received from $(A)$ by setting the key share $\alpha_0$ to $\alpha_1=g^{xb_0}$. The process goes as follows:
\begin{figure}[H]
    \centering

           \begin{tabular}{| m{0.1\textwidth} | m{0.1\textwidth} | m{0.45\textwidth} |}
            \hline
            \multicolumn{3}{|c|}{\centering{$\beta_i$}}\\
            \hline
            $n_{i+1}$ & $\gamma_{i+1}$& $\beta_{i+1}$ \\
           \hline
           \end{tabular}                   
    \end{figure}
\hspace{-5mm}The first $\kappa$ bits of $\beta_i$ will be $n_{i+1}$ itself, the next $\kappa$ bits will be $\gamma_{i+1}$, and the remaining $(2r-1)\kappa$ bits of $\beta_i$ are shifted left to form the leftmost $(2r-1)\kappa$ bits of $\beta_{i+1}$; the rightmost $2\kappa$ bits of $\beta_{i+1}$ are simply a substring of an output of the PRNG function.
