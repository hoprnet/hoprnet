...
\begin{itemize}
    \item The signature of the ticket issuer is valid if the ECDSA public key recovery function results in $address(T_{c_i})$
    ...
    \item Channel balance towards the ticket recipient is sufficient to cover the costs for the ticket: $$ channel.balance>F$$ where $F$ is the relay fee for a ticket 
