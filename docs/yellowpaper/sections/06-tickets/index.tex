...
\\$(A)$ does not know whether the ticket is a win or not.
    \\$(A)$ sets content of a ticket to: $$t=(\sigma,P_w,R,\alpha,I,T_c,\zeta)$$
\\~\\$(A)$ then signs the ticket with its private key and sends $T= (t, Sig_I(t))$ to the recipient together with a mixnet packet.
    \\~\\\textbf{Recipient's Ethereum adress $R$:} is a unique identifier that is derived from the recipient's public key.
    ...
    \\\textbf{Channel Epoch $\zeta$:} is used to give each reincarnation of the payment channel a new identifier such that tickets issued for previous instances of the channel lose their validity once a channel is reopened. This is due to the fact that $\zeta$ increments whenever a closed channel is re(-opened).
    \begin{comment}
...
Additionally, the node retrieves the commitment opening value $open$ to the current on-chain commitment (used to verify whether a ticket is a win) and checks whether $r$, $open$ leads to a winning ticket.
...
\begin{itemize}
    ...
    \item Ticket Epoch $\alpha$ must match with the spending channel ticket epoch $$\alpha=\alpha_c$$
    \item The ticket has been already redeemed and ticket index is strictly greater than the current value in the smart contract (replay protection). This is valid because redeeming a $ticket$ with index $I_c=n$ requires $I_c < n+1$ and sets $I= n+1$. So the new index becomes greater than $n$.
     $$I_c <I$$ where $I_c$ is the current value in the spending channel and $I$ the ticket index.
