...
\\~\\Cover traffic also increases the anonymity level of the mix network by making it harder to identify whether a specific node is a sender communicating with a receiver. This is also known as sender online unobservability, one of the main goals outlined in the \nameref{sec:securitygoals} section.
...
After opening channels, the CT node sends cover traffic packets at regular intervals. The path selection algorithm used for cover traffic is the same one used to select nodes for real traffic (see the \nameref{sec:path-selection} section for more details) where nodes are selected at random but weighted by importance, starting at the CT node.
...
\\~\\ As with real traffic, rewards are distributed to all selected nodes in a path as tickets which can be redeemed for HOPR tokens with a certain probability (see the \nameref{sec:tickets} section). Thus there is a direct correlation between a node's importance and the cover traffic rewards it receives, ensuring that rewards are distributed fairly.
