...
\begin{tikzpicture}
    ...
    \begin{scope}[shift={(0.5*\coreWidth+\otherNodeOffset,0)}]
        \draw (1,\connectHeight+\padding+1.35) node {\includegraphics[width=40pt]{images/ethereum.pdf}};
        \draw[dashed] (0,0) rectangle (\ethereumWidth,\connectHeight+2.5+2*\padding);
    ...
    \begin{scope}[shift={(-\otherNodeOffset+1*\padding,0)}]
        \draw[decoration={brace,raise=5pt},decorate] (0,0) -- (0,\coreHeight) node[midway,left=7pt] {\rotatebox{90}{packet}};
        \draw[decoration={brace,raise=5pt},decorate] (0,\coreHeight+\padding) -- (0,\coreHeight+\padding+\connectHeight) node[midway,left=7pt] {\rotatebox{90}{message}};
    \end{scope}

    % network <-> incentives
    \draw[decoration={brace,raise=5pt,mirror},decorate] (-\otherNodeOffset+1*\padding,0) -- (\coreWidth*0.5-\padding*0.5,0) node[midway,below=6pt] {network};
    \draw[decoration={brace,raise=5pt,mirror},decorate] (\coreWidth*0.5+\padding*0.5,0) -- (0.5*\coreWidth+\otherNodeOffset+\ethereumWidth,0) node[midway,below=6pt] {incentives};
