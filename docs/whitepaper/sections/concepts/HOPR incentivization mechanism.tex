...
\subsubsection{Types of HOPR nodes}
We have three types of nodes in the HOPR network:
\begin{itemize}
    \item \textbf{(Hubs)} Nodes that are highly available, have high computational power and network bandwidth and advertising their availability 
    through highly staked payment channels.
    \item \textbf{(Relayers)} Nodes that have a few medium-staked payment channels to other nodes, including payment channels to Hubs.
    \item \textbf{(End users)} Nodes that run on small hardware or in the browser, potentially without a synchronized blockchain, few payment channels to other user, low stake.
\end{itemize}
