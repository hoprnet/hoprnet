...
\\$(A)$ sets content of a ticket to: $$t:=(\delta,P_w,R,I,C,\zeta,c_{Id},tag,V)$$ 
where $\delta$ is the ticket amount, $P_w$ is the winning probability, $R$ is the recipient's adress on the blockchain (hashed public key into 20 bytes), $I$ is the ticket index which is set by the ticket issuer and should be increased with every issued ticket, $C$ is the challenge, $\alpha_c$ is the account.counter, $\zeta$ is the channel.iteration, $c_{Id},tag,V$ are ChainId, HOPR tag and HOPR version which are given as constants and can depend on the utilized blockchain.
...
This is the case if $$H( ticketHash, response, open ) <P_w$$ where $$ticketHash:=H(t)$$
...
\begin{itemize}
    \item The signature of the ticket issuer is valid: $$ecrecover(Sig_{response}(0))=address(t.challenge)$$
    \item The ticket has been already spent (replay protection): $$channel.index <t.index$$
