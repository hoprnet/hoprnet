...
Packet Key
: An ed25519 key which is used by a single node. Its generated by
the node itself at initialization of the node. The packet key is used for
HOPR packet-related cryptographic operations.

Chain Key
: An secp256k1 key which is used by a single node. Its generated by
...
2. Node submits transaction to `NodeSafe` as delegate, transaction is then
