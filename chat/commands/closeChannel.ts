...
export default class CloseChannel implements AbstractCommand {
    ...
    async execute(query?: string): Promise<any> {
        ...
        try {
            channel = await this.node.paymentChannels.channel
                .create(this.node.paymentChannels, peerId.pubKey.marshal(), async (counterparty: Uint8Array) => this.node.interactions.payments.onChainKey.interact(await pubKeyToPeerId(counterparty)))

            await channel.initiateSettlement()

            console.log(`${chalk.green(`Successfully closed channel`)} ${chalk.yellow(u8aToHex(await channel.channelId))}. Received ${chalk.magenta(new BN(0).toString())} ${this.node.paymentChannels.types.Balance.SYMBOL}.`)
        } catch (err) {
