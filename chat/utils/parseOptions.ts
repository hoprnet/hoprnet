...
export async function parseOptions(): Promise<HoprOptions> {
    ...
    try {
        ...
        console.log(`Could not find <${chalk.red(`@hoprnet/hopr-core-${cli_options.network}`)}>. Please make sure it is available under ./node_modules!\n`)
        await listConnectors.execute()
        return
    }
    ...
    let addr: Multiaddr, bootstrapServerMap = new Map<string, PeerInfo>()
    ...
    if (process.env[`${cli_options.network.toUpperCase()}_PROVIDER`] == null) {
        throw Error(`Could not find any connector for ${chalk.magenta(cli_options.network)}. Please specify ${chalk.yellow(`${cli_options.network.toUpperCase()}_PROVIDER`)} in ${chalk.yellow('.env')}.`)
    }
