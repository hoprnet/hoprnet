...
usage() {
  msg
  msg "Usage: $0 <host> <rest_port> <ws_port> <insecure_ws_port> <admin_port> <insecure_admin_port> <api_token>"
  msg
}
...
declare rest_port="${2}"
declare ws_port="${3}"
declare insecure_ws_port="${4}"
declare admin_port="${5}"
declare insecure_admin_port="${6}"
declare api_token="${7}"

log "Security tests started"
log "Rest API @ ${host}:${rest_port}"
log "websocket API @ ${host}:${ws_port}"
log "No-auth websocket API @ ${host}:${insecure_ws_port}"
...
wait_for_port "${ws_port}" "${host}"
wait_for_port "${insecure_ws_port}" "${host}"
...
testWebsocketSecurity() {
  ...
  log "${name} should reject commands without token"
  ws_response=$(echo "info" | websocat ws://${host}:${port}${path})
  ...
  log "${name} should reject commands with invalid token"
  ws_response=$(echo "info" | websocat ws://${host}:${port}${path} --header "Cookie:X-Auth-Token=${bad_token}")
  msg_type=$(echo "${ws_response}" | jq .type --raw-output )
  ...
  log "${name} should execute info command with correct token"
  ws_response=$(echo "info" | websocat ws://${host}:${port}${path} -0 --header "Cookie:X-Auth-Token=${api_token}")
  ...
  ws_response=$(echo "info" | websocat ws://${host}:${insecure_port}${path} -0)
  ...
  ws_response=$(echo "info" | websocat ws://${host}:${insecure_port}${path} -0 --header "Cookie:${bad_token}")
  if [[ "${ws_response}" != *"ws client connected [ authentication DISABLED ]"* ]]; then
    log "⛔️ Didn't succeed ws authentication"
    log "Expected response should contain: 'ws client connected [ authentication DISABLED ]' "
    log "Actual response:"
...
testWebsocketSecurity "websocket V1" "/" $ws_port $insecure_ws_port
testWebsocketSecurity "websocket V2" "/v2" $ws_port $insecure_ws_port
