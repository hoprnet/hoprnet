...
| 2   | `impls/strategy/src/strategy.rs`         | 73   | 🟠 Major | **Discard strategies on error.** Without `runtime-tokio`, the drain drops all sub-strategies then reports success.                                                            | **Fixed.** Removed unconditional `features = ["runtime-tokio"]` from `[dependencies]` — `hopr-utils` now only gets `runtime-tokio` through `hopr-strategy`'s own feature. Added `#[cfg]` guard to `MultiStrategy::run()` that returns an explicit runtime error when the feature is off. |
| 3   | `protocols/pix/src/reconstructor/mod.rs` | 124  | 🟠 Major | **Bound caches by global protocol state, not per-SSA constants.** At max polynomial count, `ssa_verifiers` retains only four complete SSA sets.                               | **Fixed.** Replaced `MAX_POLYS_PER_SSA * 4` with `MAX_CONCURRENT_SSA_CYCLES * MAX_POLYS_PER_SSA`, sized for 8 concurrent SSA cycles (vs 4).                                                                                                                                              |
...
| --- | --------------------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A   | `protocols/pix/src/reconstructor/mod.rs`      | 135-149   | Share removed from cache before verifier confirmed. `awaiting_ack_from_peer.remove()` consumes ciphertext before `ssa_verifiers.get()` confirms verifier exists. On `MissingVerifier` the share is permanently lost. Fix plan exists in `plans/fix-share-removed-before-verifier.md`. | **Fixed.** `.get()` before `.remove()` in `process_verified_ack` so share survives `MissingVerifier`. Added `pending_ack_keys` cache: stashed `(challenge, ack)` pairs survive across `acknowledge_shares` calls, drained at the start of each call for retry when the verifier arrives. `MissingVerifier` downgraded from `error!` to `trace!`. |
| B   | `transport/hopr/src/protocol/pipeline/mod.rs` | 909-916   | Dual-role Relay+Exit nodes don't register outgoing PIX shares. `(node_type == NodeType::Exit).then(\|\| exit_ack_proc.clone())` at line 915 misses the Relay+Exit case.                                                                                                               | **Fixed.** Changed condition from `node_type == NodeType::Exit` to `node_type != NodeType::Entry`. Entry is the only role that never has an exit_ack_proc; for pure Relay the builder supplies a no-op, making the guard safe regardless of whether a real processor exists. |
| C   | `transport/session/src/manager.rs`            | 1500-1575 | Kill-switch armed after SSA request is sent. SSA request at line ~1547 before deposit-timeout kill switch at line ~1557 — window with no enforcement if setup fails.                                                                                                                  | **Fixed.** Kill-switch installed before SSA request is sent. The deposit-timeout timer is now registered in the abort handles before the request goes out on the wire, closing the window.                                                                                   |
