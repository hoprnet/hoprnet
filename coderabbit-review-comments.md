...
| 2 | `impls/strategy/src/strategy.rs` | 73 | 🟠 Major | **Discard strategies on error.** Without `runtime-tokio`, the drain drops all sub-strategies then reports success. | **Outdated.** The `#[cfg(not(feature = "runtime-tokio"))]` branch that silently drained strategies was removed in a prior refactor. Current `run()` always spawns sub-strategies via `hopr_utils::runtime::prelude::spawn`. |
