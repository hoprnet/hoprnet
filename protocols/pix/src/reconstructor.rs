...
use hopr_types::{
    crypto::prelude::{HalfKeyChallenge},
    internal::prelude::VerifiedAcknowledgement
};
use vsss_rs::{ReadableShareSet, elliptic_curve::{CurveArithmetic, Group, group::Curve}};

use crate::{PixSpec, Scalar, errors, Element, SsaPolynomialIndex, PartialSsaShareVerifier, PartialSsaShare, complete_share, CompletedShare, types::{EncryptedPartialSsaShare, SsaIndex}};
...
impl<S: PixSpec> SsaBuilder<S> {
    pub fn new(commitment: Element<S>, num_parts: NonZeroUsize) -> Self {
        Self { commitment: commitment.to_affine(), num_parts: num_parts.get(), builder: Scalar::<S>::default() }
    }

    pub fn add_ssa_part(&mut self, sub_secret: Scalar<S>) -> errors::Result<Option<Scalar<S>>> {
        match self.num_parts.checked_sub(1) {
            Some(n) => {
...
impl<S: PixSpec> SsaPartReconstructor<S> {
    pub fn new(verifier: PartialSsaShareVerifier<S>) -> Self {
        Self { verifier, shares: Vec::new() }
    }

    pub fn add_share(&mut self, spi: SsaPolynomialIndex<S::Pseudonym>, msg: impl AsRef<[u8]>, share: PartialSsaShare<S>) -> errors::Result<Option<Scalar<S>>> {
...
impl<S: PixSpec> Clone for AwaitingPartialShare<S> {
    fn clone(&self) -> Self {
        Self { spi: self.spi.clone(), msg: self.msg.clone(), enc_share: self.enc_share.clone() }
    }
}

pub struct SsaReconstructor<S: PixSpec> {
    channel: (async_broadcast::Sender<Scalar<S>>, async_broadcast::InactiveReceiver<Scalar<S>>),
    ssa_builders: moka::sync::Cache<SsaIndex, std::sync::Arc<parking_lot::Mutex<SsaBuilder<S>>>>,
    ssa_verifiers: moka::sync::Cache<SsaPolynomialIndex<S::Pseudonym>, std::sync::Arc<parking_lot::Mutex<SsaPartReconstructor<S>>>>,
...
impl<S: PixSpec + 'static> SsaReconstructor<S> {
    pub fn add_ssa_commitment(&self, ssa_index: SsaIndex, num_parts: NonZeroUsize, ssa_commitment: Element<S>) {
        self.ssa_builders.insert(ssa_index, std::sync::Arc::new(parking_lot::Mutex::new(SsaBuilder::new(ssa_commitment, num_parts))));
    }

    pub fn add_verifier(&self, verifier: PartialSsaShareVerifier<S>) {
        self.ssa_verifiers.insert(verifier.spi, std::sync::Arc::new(parking_lot::Mutex::new(SsaPartReconstructor::new(verifier))));
    }

    pub fn add_share(&self, challenge: HalfKeyChallenge, spi: SsaPolynomialIndex<S::Pseudonym>, msg: impl AsRef<[u8]>,  enc: EncryptedPartialSsaShare<S>) -> errors::Result<()> {
        self.awaiting_acks.insert(challenge, AwaitingPartialShare { spi, msg: msg.as_ref().into(), enc_share: enc });
        Ok(())
    }
    ...
    pub fn new_acknowledgement(&self, ack: VerifiedAcknowledgement) -> errors::Result<()> {
        ...
        let reconstructor = self.ssa_verifiers.get(&share.spi).ok_or(errors::PixError::MissingVerifier)?;

        let partial_share = share.enc_share.decrypt(ack.ack_key_share());
        ...
        let builder = self.ssa_builders.get(&share.spi.ssa_index()).ok_or(errors::PixError::MissingSsaCommitment)?;
