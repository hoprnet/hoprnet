...
class TicketUtils {
    ...
    async create(amount, challenge, arr) {
        const account = await this._channel.coreConnector.utils.pubKeyToAccountId(this._channel.counterparty);
        const { hashedSecret } = await this._channel.coreConnector.hoprChannels.methods.accounts(hopr_utils_1.u8aToHex(account)).call();
        ...
        const channelId = await this._channel.channelId;
        ...
        await utils_1.sign(await ticket.hash, this._channel.coreConnector.self.privateKey, undefined, {
            bytes: signedTicket.buffer,
    ...
    async verify(signedTicket) {
        ...
        // }
        try {
            await this._channel.testAndSetNonce(signedTicket);
        }
        catch {
            return false;
        }
        return utils_1.verify(await signedTicket.ticket.hash, signedTicket.signature, await this._channel.offChainCounterparty);
    }
    ...
    get SIZE() {
        return types_1.Ticket.SIZE;
    }
}
exports.default = TicketUtils;
