...
class Channel {
    ...
    // }
    async onceClosed() {
        return onceClosed(this.coreConnector, await this.coreConnector.account.address, await this.coreConnector.utils.pubKeyToAccountId(this.counterparty));
    }
    ...
    handleOpeningRequest() {
        return (source) => async function* () {
            for await (const _msg of source) {
                ...
                onceOpen(this.coreConnector, await this.coreConnector.account.address, counterparty).then(() => onOpen(this.coreConnector, counterpartyPubKey, signedChannel));
