...
const onceOpen = async (coreConnector, self, counterparty) => {
    ...
    return new Promise((resolve, reject) => {
        const onData = async (data) => {
            ...
            const _channelId = await coreConnector.utils.getId(hopr_utils_1.stringToU8a(opener), hopr_utils_1.stringToU8a(counterParty));
...
const onceClosed = async (coreConnector, self, counterparty) => {
    ...
    return new Promise((resolve, reject) => {
        const onData = async (data) => {
            ...
            const _channelId = await coreConnector.utils.getId(hopr_utils_1.stringToU8a(closer), hopr_utils_1.stringToU8a(counterParty));
            if (!hopr_utils_1.u8aEquals(_channelId, channelId)) {
                return;
            }
            resolve(data.returnValues);
...
class Channel {
    ...
    async getPreviousChallenges() {
        ...
        return new Promise(async (resolve, reject) => {
            this.coreConnector.db
                ...
                pubKeys.push(new Uint8Array(hopr_utils_1.u8aXOR(false, challenge, ownKeyHalf.toU8a())));
            })
                .on('end', () => {
                if (pubKeys.length > 0) {
    ...
    static async isOpen(coreConnector, counterpartyPubKey) {
        ...
        if (onChain != offChain) {
            if (!onChain && offChain) {
                console.log(`Channel ${hopr_utils_1.u8aToHex(channelId)} exists off-chain but not on-chain, deleting data.`);
    ...
    static async create(coreConnector, counterpartyPubKey, _getOnChainPublicKey, channelBalance, sign) {
        ...
        if (await this.isOpen(coreConnector, counterpartyPubKey)) {
            ...
            await Channel.increaseFunds(coreConnector, new types_1.AccountId(hopr_utils_1.stringToU8a(spender)), counterparty, amount);
                ...
                for await (const _msg of source) {
                    ...
                    if (coreConnector.utils.isPartyA(coreConnector.account, counterparty)) {
                        if (channelBalance.balance.sub(channelBalance.balance_a).gtn(0)) {
                            await Channel.increaseFunds(coreConnector, new types_1.AccountId(hopr_utils_1.stringToU8a(spender)), counterparty, new types_1.Balance(channelBalance.balance.sub(channelBalance.balance_a)));
                        }
                    }
                    else {
                        if (channelBalance.balance_a.gtn(0)) {
                            await Channel.increaseFunds(coreConnector, new types_1.AccountId(hopr_utils_1.stringToU8a(spender)), counterparty, channelBalance.balance_a);
                        }
                    }
