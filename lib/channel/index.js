...
const onceOpen = async (coreConnector, self, counterparty) => {
    ...
    return new Promise((resolve, reject) => {
        const onData = async (data) => {
            ...
            const _channelId = await coreConnector.utils.getId(u8a_1.stringToU8a(opener), u8a_1.stringToU8a(counterParty));
            if (!u8a_1.u8aEquals(_channelId, channelId)) {
                return;
            }
            resolve(data.returnValues);
        };
        const onError = (error) => reject(error);
        events.addEvent('OpenedChannel', coreConnector.hoprChannels.events
            .OpenedChannel({
            filter: {
                opener: self.toHex(),
...
const onceClosed = async (coreConnector, self, counterparty) => {
    ...
    return new Promise((resolve, reject) => {
        const onData = async (data) => {
            ...
            const _channelId = await coreConnector.utils.getId(u8a_1.stringToU8a(closer), u8a_1.stringToU8a(counterParty));
            if (!u8a_1.u8aEquals(_channelId, channelId)) {
                return;
            }
            resolve(data.returnValues);
        ...
        const onError = (error) => reject(error);
        events.addEvent('ClosedChannel', coreConnector.hoprChannels.events
            .ClosedChannel({
            filter: {
                opener: self.toHex(),
                counterParty: counterparty.toHex()
...
class Channel {
    ...
    async onceOpen() {
        return onceOpen(this.coreConnector, this.coreConnector.account, await this.coreConnector.utils.pubKeyToAccountId(this.counterparty));
    }
    async onceClosed() {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                const response = await this.coreConnector.hoprToken.methods
                    .balanceOf(u8a_1.u8aToHex(this.coreConnector.account))
                    .call();
    ...
    get currentBalanceOfCounterparty() {
        return new Promise(async (resolve, reject) => {
            try {
                const response = await this.coreConnector.hoprToken.methods
                    .balanceOf(u8a_1.u8aToHex(await this.coreConnector.utils.pubKeyToAccountId(this.counterparty)))
                    .call();
                return resolve(new types_1.Balance(response));
            }
            catch (error) {
    ...
    async initiateSettlement() {
        ...
        try {
            ...
            if (status === channel_1.ChannelStatus.OPEN) {
                await utils_1.waitForConfirmation((await this.coreConnector.signTransaction(this.coreConnector.hoprChannels.methods.initiateChannelClosure(u8a_1.u8aToHex(await this.coreConnector.utils.pubKeyToAccountId(this.counterparty))), {
                ...
                await utils_1.waitForConfirmation((await this.coreConnector.signTransaction(this.coreConnector.hoprChannels.methods.claimChannelClosure(u8a_1.u8aToHex(await this.coreConnector.utils.pubKeyToAccountId(this.counterparty))), {
                    from: this.coreConnector.account.toHex(),
                    to: this.coreConnector.hoprChannels.options.address
                })).send());
            }
    ...
    async getPreviousChallenges() {
        ...
        return new Promise(async (resolve, reject) => {
            this.coreConnector.db
                .createReadStream({
                gt: Buffer.from(this.coreConnector.dbKeys.Challenge(await this.channelId, new Uint8Array(constants_1.HASH_LENGTH).fill(0x00))),
                lt: Buffer.from(this.coreConnector.dbKeys.Challenge(await this.channelId, new Uint8Array(constants_1.HASH_LENGTH).fill(0xff)))
            })
                .on('error', reject)
                ...
                pubKeys.push(new Uint8Array(u8a_1.u8aXOR(false, challenge, ownKeyHalf.toU8a())));
            })
                .on('end', () => {
                if (pubKeys.length > 0) {
                    return resolve(new types_1.Hash(u8a_1.u8aXOR(false, ...pubKeys)));
                }
                resolve();
            });
        });
    ...
    static async isOpen(coreConnector, counterpartyPubKey) {
        ...
        if (onChain != offChain) {
            if (!onChain && offChain) {
                console.log(`Channel ${u8a_1.u8aToHex(channelId)} exists off-chain but not on-chain, deleting data.`);
                await onClose(coreConnector, counterpartyPubKey);
            }
            else {
                throw Error(`Channel ${u8a_1.u8aToHex(channelId)} exists on-chain but not off-chain.`);
            }
        }
        return onChain && offChain;
    }
    static async increaseFunds(coreConnector, spender, counterparty, amount) {
        try {
            ...
            const allowance = await coreConnector.hoprToken.methods
                ...
                .call()
                .then(v => new bn_js_1.default(v));
            if (allowance.isZero()) {
                await utils_1.waitForConfirmation((await coreConnector.signTransaction(coreConnector.hoprToken.methods.approve(spender.toHex(), amount.toString()), {
                    from: coreConnector.account.toHex(),
                    to: coreConnector.hoprToken.options.address
                })).send());
            }
            else if (allowance.lt(amount)) {
                await utils_1.waitForConfirmation((await coreConnector.signTransaction(coreConnector.hoprToken.methods.increaseAllowance(spender.toHex(), amount.sub(allowance).toString()), {
                    from: coreConnector.account.toHex(),
                    to: coreConnector.hoprToken.options.address
                })).send());
            }
            await utils_1.waitForConfirmation((await coreConnector.signTransaction(coreConnector.hoprChannels.methods.fundChannel(coreConnector.account.toHex(), counterparty.toHex(), amount.toString()), {
                from: coreConnector.account.toHex(),
    ...
    static async create(coreConnector, counterpartyPubKey, _getOnChainPublicKey, channelBalance, sign) {
        ...
        if (await this.isOpen(coreConnector, counterpartyPubKey)) {
            ...
            await Channel.increaseFunds(coreConnector, new types_1.AccountId(u8a_1.stringToU8a(spender)), counterparty, amount);
    ...
    static getAll(coreConnector, onData, onEnd) {
        ...
        return new Promise((resolve, reject) => {
            coreConnector.db
                .createReadStream({
                gt: Buffer.from(coreConnector.dbKeys.Channel(new Uint8Array(types_1.Hash.SIZE).fill(0x00))),
                lt: Buffer.from(coreConnector.dbKeys.Channel(new Uint8Array(types_1.Hash.SIZE).fill(0xff)))
            })
                ...
                for await (const _msg of source) {
                    ...
                    if (coreConnector.utils.isPartyA(coreConnector.account, counterparty)) {
                        if (channelBalance.balance.sub(channelBalance.balance_a).gtn(0)) {
                            await Channel.increaseFunds(coreConnector, new types_1.AccountId(u8a_1.stringToU8a(spender)), counterparty, new types_1.Balance(channelBalance.balance.sub(channelBalance.balance_a)));
                        }
                    }
                    else {
                        if (channelBalance.balance_a.gtn(0)) {
                            await Channel.increaseFunds(coreConnector, new types_1.AccountId(u8a_1.stringToU8a(spender)), counterparty, channelBalance.balance_a);
                        }
                    }
