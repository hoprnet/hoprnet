...
class TicketFactory {
    ...
    async create(amount, challenge, arr) {
        ...
        const counterParty = (await utils_1.pubKeyToAccountId(this.channel.counterparty)).toHex();
        const { onChainSecret, epoch } = await this.channel.coreConnector.hoprChannels.methods
            .accounts(counterParty)
            .call()
            .then((res) => {
            return {
                onChainSecret: hopr_utils_1.stringToU8a(res.hashedSecret),
    ...
    async submit(signedTicket, secretA, secretB) {
        ...
        const transaction = await signTransaction(hoprChannels.methods.redeemTicket(hopr_utils_1.u8aToHex(preImage.preImage), hopr_utils_1.u8aToHex(ticket.channelId), hopr_utils_1.u8aToHex(await utils.hash(hopr_utils_1.u8aConcat(secretA, secretB))), ticket.amount.toString(), hopr_utils_1.u8aToHex(ticket.winProb), hopr_utils_1.u8aToHex(r), hopr_utils_1.u8aToHex(s), v + 27), {
