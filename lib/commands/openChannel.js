...
class OpenChannel {
    ...
    async execute(rl, query) {
        ...
        const tokens = new bignumber_js_1.default((await this.node.paymentChannels.account.balance).toString()).div(new bignumber_js_1.default(10).pow(this.node.paymentChannels.types.Balance.DECIMALS));
        let funds, tmpFunds;
        ...
        try {
            await this.node.paymentChannels.channel.create(counterparty.pubKey.marshal(), async () => this.node.paymentChannels.utils.pubKeyToAccountId(await this.node.interactions.payments.onChainKey.interact(counterparty)), channelBalance, (balance) => this.node.interactions.payments.open.interact(counterparty, balance));
    ...
    async complete(line, cb, query) {
        const peersWithOpenChannel = await utils_1.getOpenChannels(this.node, this.node.peerInfo.id);
        const allPeers = utils_1.getPeers(this.node, {
            noBootstrapNodes: true,
        });
        const peers = allPeers.reduce((acc, peer) => {
            if (!peersWithOpenChannel.find((p) => p.id.equals(peer.id))) {
