...
class ListOpenChannels {
    ...
    async execute() {
        ...
        try {
            await this.node.paymentChannels.channel.getAll(async (channel) => {
                const channelId = await channel.channelId;
                if (!channel.counterparty) {
                    str += `${chalk_1.default.yellow(hopr_utils_1.u8aToHex(channelId))} - ${chalk_1.default.gray('pre-opened')}`;
                    return;
                }
                const peerId = await utils_1.pubKeyToPeerId(await channel.offChainCounterparty);
                str += `${chalk_1.default.yellow(hopr_utils_1.u8aToHex(channelId))} - ${chalk_1.default.blue(peerId.toB58String())}\n`;
                return;
            }, async (promises) => {
...
//# sourceMappingURL=listOpenChannels.js.map