...
class HoprEthereum {
    ...
    get accountBalance() {
        return this.hoprToken.methods
            .balanceOf(u8a_1.u8aToHex(this.account))
            .call()
            .then(res => {
            return new bn_js_1.default(res);
        });
    }
    async start() {
    ...
    async checkAccountSecret() {
        ...
        onChainSecret = await this.hoprChannels.methods
            ...
            .call()
            .then(res => u8a_1.stringToU8a(res.hashedSecret))
            .then(secret => {
            if (u8a_1.u8aEquals(secret, new Uint8Array(types.Hash.SIZE).fill(0x00))) {
                return undefined;
            }
            return secret;
        });
        ...
        if (hasOffChainSecret !== hasOnChainSecret) {
            if (hasOffChainSecret) {
                ...
                await utils.waitForConfirmation((await this.signTransaction(this.hoprChannels.methods.setHashedSecret(u8a_1.u8aToHex(offChainSecret)), {
    ...
    async setAccountSecret(nonce) {
        ...
        await Promise.all([
            await utils.waitForConfirmation((await this.signTransaction(this.hoprChannels.methods.setHashedSecret(u8a_1.u8aToHex(secret)), {
                from: this.account.toHex(),
                to: this.hoprChannels.options.address,
    ...
    static async create(db, seed, options) {
        ...
        const privateKey = usingSeed ? seed : u8a_1.stringToU8a(config.DEMO_ACCOUNTS[options.id]);
