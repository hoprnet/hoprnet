    ...
    async execute(query) {
        ...
        try {
            channel = await this.node.paymentChannels.channel
                .create(this.node.paymentChannels, peerId.pubKey.marshal(), async (counterparty) => this.node.interactions.payments.onChainKey.interact(await utils_2.pubKeyToPeerId(counterparty)));
