...
function toLengthPrefixedU8a(arg, additionalPadding, length) {
    if (additionalPadding != null) {
        if (length != null && arg.length + _1.LENGTH_PREFIX_LENGTH + additionalPadding.length > length) {
            throw Error(`Cannot create length-prefixed ${Uint8Array.name} because encoded ${Uint8Array.name} would be ${length - arg.length + _1.LENGTH_PREFIX_LENGTH + additionalPadding.length} bytes greater than desired size of ${length} bytes.`);
        }
        if (length != null) {
        ...
        if (length != null && arg.length + _1.LENGTH_PREFIX_LENGTH > length) {
            throw Error(`Cannot create length-prefixed ${Uint8Array.name} because encoded ${Uint8Array.name} would be ${length - arg.length + _1.LENGTH_PREFIX_LENGTH} bytes greater than desired size of ${length} bytes.`);
        }
        if (length != null) {
