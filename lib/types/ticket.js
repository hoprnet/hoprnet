...
class Ticket extends extended_1.Uint8ArrayE {
    ...
    static async create(amount, challenge, arr) {
        const account = await this.coreConnector.utils.pubKeyToAccountId(this.counterparty);
        const { hashedSecret } = await this.coreConnector.hoprChannels.methods.accounts(hopr_utils_1.u8aToHex(account)).call();
        ...
        const signedTicket = new _1.SignedTicket(arr);
        ...
        await utils_1.sign(await ticket.hash, this.coreConnector.self.privateKey, undefined, {
            bytes: signedTicket.buffer,
            offset: signedTicket.signatureOffset,
        });
        return signedTicket;
    }
    static async verify(signedTicket) {
        ...
        return utils_1.verify(await signedTicket.ticket.hash, signedTicket.signature, await this.offChainCounterparty);
    }
    // @TODO: implement submit
    static async submit(signedTicket) {
