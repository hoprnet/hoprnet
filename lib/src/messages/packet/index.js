...
class Packet extends Uint8Array {
    ...
    static async create(node, msg, path) {
        ...
        node.log(`Encrypting with ${node.paymentChannels.utils.hash(hopr_utils_1.u8aXOR(false, header_1.deriveTicketKey(secrets[0]), header_1.deriveTicketKey(secrets[1]))).toString()}.`);
