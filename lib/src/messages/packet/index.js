...
class Packet extends Uint8Array {
    constructor(node, arr, struct) {
        if (arr != null && struct == null) {
            ...
            super(hopr_utils_1.u8aConcat(struct.header, struct.ticket, struct.challenge, struct.message));
        }
        else {
    ...
    static async create(node, msg, path) {
        ...
        node.log(`Encrypting with ${(await node.paymentChannels.utils.hash(secrets.length == 1
            ? header_1.deriveTicketLastKey(secrets[0])
            : secp256k1_1.default.privateKeyTweakAdd(header_1.deriveTicketKey(secrets[0]), header_1.deriveTicketKey(secrets[1])))).toString()}.`);
        ...
        packet.ticket.set(await channel.ticket.create(channel, fee, secrets.length == 1
            ? header_1.deriveTicketLastKey(secrets[0])
            : secp256k1_1.default.privateKeyTweakAdd(header_1.deriveTicketKey(secrets[0]), header_1.deriveTicketKey(secrets[1]))));
        return packet;
    }
    /**
