...
class Packet extends Uint8Array {
    ...
    static async create(node, msg, path) {
        ...
        const ticketChallenge = await node.paymentChannels.utils.hash(secrets.length == 1
            ...
            : hopr_utils_1.u8aConcat((header_1.deriveTicketKey(secrets[0]),
                await node.paymentChannels.utils.hash(header_1.deriveTicketKeyBlinding(secrets[1])))));
    ...
    async prepareForward(state, newState, target) {
        ...
        this._challenge = await challenge_1.Challenge.create(this.node.paymentChannels, this.header.hashedKeyHalf, forwardedFunds, {
            bytes: this.buffer,
            offset: this.challengeOffset,
