...
class Header extends Uint8Array {
    ...
    extractHeaderInformation(lastNode = false) {
        ...
        if (lastNode) {
            const { key, iv } = derivePRGParameters(this.derivedSecret);
            this.tmpData.set(hopr_utils_1.u8aXOR(false, this.beta.subarray(0, parameters_1.DESINATION_SIZE + parameters_1.IDENTIFIER_SIZE), utils_1.PRG.createPRG(key, iv).digest(0, parameters_1.DESINATION_SIZE + parameters_1.IDENTIFIER_SIZE)), 0);
        }
        else {
            ...
            hopr_utils_1.u8aXOR(true, tmp, utils_1.PRG.createPRG(key, iv).digest(0, exports.BETA_LENGTH + parameters_1.PER_HOP_SIZE));
