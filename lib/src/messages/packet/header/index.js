...
class Header extends Uint8Array {
    ...
    verify() {
        return utils_1.u8aEquals(createMAC(this.derivedSecret, this.beta), this.gamma);
    }
    extractHeaderInformation(lastNode = false) {
        const { key, iv } = derivePRGParameters(this.derivedSecret);
        if (lastNode) {
            const { key, iv } = derivePRGParameters(this.derivedSecret);
            this.tmpData.set(utils_1.u8aXOR(false, this.beta.subarray(0, parameters_1.DESINATION_SIZE + parameters_1.IDENTIFIER_SIZE), utils_1.PRG.createPRG(key, iv).digest(0, parameters_1.DESINATION_SIZE + parameters_1.IDENTIFIER_SIZE)), 0);
        }
        else {
            ...
            utils_1.u8aXOR(true, tmp, utils_1.PRG.createPRG(key, iv).digest(0, exports.BETA_LENGTH + parameters_1.PER_HOP_SIZE));
    ...
    toString() {
        return ('Header:\n' +
            '|-> Alpha:\n' +
            '|---> ' +
            utils_1.u8aToHex(this.alpha) +
            '\n' +
            '|-> Beta:\n' +
            '|---> ' +
            utils_1.u8aToHex(this.beta) +
            '\n' +
            '|-> Gamma:\n' +
            '|---> ' +
            utils_1.u8aToHex(this.gamma) +
            '\n');
    }
    static get SIZE() {
        return parameters_1.COMPRESSED_PUBLIC_KEY_LENGTH + exports.BETA_LENGTH + parameters_1.MAC_SIZE;
    }
    static async create(node, peerIds) {
        const header = new Header({ bytes: new Uint8Array(Header.SIZE).buffer, offset: 0 });
...
function deriveCipherParameters(secret) {
    ...
    const keyAndIV = futoin_hkdf_1.default(Buffer.from(secret), utils_1.PRP.KEY_LENGTH + utils_1.PRP.IV_LENGTH, { salt: HASH_KEY_PRP });
    const key = keyAndIV.subarray(0, utils_1.PRP.KEY_LENGTH);
    const iv = keyAndIV.subarray(utils_1.PRP.KEY_LENGTH);
    return { key, iv };
}
...
function derivePRGParameters(secret) {
    ...
    const keyAndIV = futoin_hkdf_1.default(Buffer.from(secret), utils_1.PRG.KEY_LENGTH + utils_1.PRG.IV_LENGTH, { salt: HASH_KEY_PRG });
    const key = keyAndIV.subarray(0, utils_1.PRG.KEY_LENGTH);
    const iv = keyAndIV.subarray(utils_1.PRG.KEY_LENGTH, utils_1.PRG.KEY_LENGTH + utils_1.PRG.IV_LENGTH);
    return { key, iv };
}
...
function deriveBlinding(alpha, secret) {
    ...
    return futoin_hkdf_1.default(Buffer.from(utils_1.u8aConcat(alpha, secret)), parameters_1.PRIVATE_KEY_LENGTH, { salt: HASH_KEY_BLINDINGS });
}
