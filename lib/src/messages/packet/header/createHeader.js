...
async function createHeader(node, header, peerIds) {
    ...
    function generateFiller(secrets) {
        ...
        for (let index = 0; index < secrets.length - 1; index++) {
            ...
            hopr_utils_1.u8aXOR(true, filler.subarray(0, length), hopr_utils_1.PRG.createPRG(key, iv).digest(start, end));
        }
        return filler;
    }
    async function createBetaAndGamma(secrets, filler, identifier) {
        ...
        for (let i = secrets.length; i > 0; i--) {
            ...
            if (i == secrets.length) {
                ...
                hopr_utils_1.u8aXOR(true, header.beta.subarray(0, parameters_1.LAST_HOP_SIZE + paddingLength), hopr_utils_1.PRG.createPRG(key, iv).digest(0, parameters_1.LAST_HOP_SIZE + paddingLength));
                ...
                hopr_utils_1.u8aXOR(true, header.beta, hopr_utils_1.PRG.createPRG(key, iv).digest(0, index_1.BETA_LENGTH));
            }
