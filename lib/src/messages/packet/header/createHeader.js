            ...
            if (i == secrets.length) {
                ...
                if (secrets.length > 2) {
                    ...
                    if (i < secrets.length - 1) {
                         ...
                         */
                        key = await node.paymentChannels.utils.hash(hopr_utils_1.u8aConcat(index_1.deriveTicketKey(secrets[i]), await node.paymentChannels.utils.hash(index_1.deriveTicketKeyBlinding(secrets[i + 1]))));
                    }
                    else if (i == secrets.length - 1) {
                        key = await node.paymentChannels.utils.hash(hopr_utils_1.u8aConcat(index_1.deriveTicketLastKey(secrets[i]), await node.paymentChannels.utils.hash(index_1.deriveTicketKeyBlinding(secrets[i]))));
                    }
