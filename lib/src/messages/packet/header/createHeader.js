...
async function createHeader(node, header, peerIds) {
    ...
    function generateFiller(secrets) {
        ...
        for (let index = 0; index < secrets.length - 1; index++) {
            ...
            utils_1.u8aXOR(true, filler.subarray(0, length), utils_1.PRG.createPRG(key, iv).digest(start, end));
        }
        return filler;
    }
    async function createBetaAndGamma(secrets, filler, identifier) {
        ...
        for (let i = secrets.length; i > 0; i--) {
            ...
            if (i == secrets.length) {
                ...
                utils_1.u8aXOR(true, header.beta.subarray(0, parameters_1.LAST_HOP_SIZE + paddingLength), utils_1.PRG.createPRG(key, iv).digest(0, parameters_1.LAST_HOP_SIZE + paddingLength));
                ...
                if (secrets.length > 2) {
                    ...
                    if (i < secrets.length - 1) {
                         ...
                         */
                        key = await node.paymentChannels.utils.hash(utils_1.u8aConcat(index_1.deriveTicketKey(secrets[i]), await node.paymentChannels.utils.hash(index_1.deriveTicketKeyBlinding(secrets[i + 1]))));
                    }
                    else if (i == secrets.length - 1) {
                        key = await node.paymentChannels.utils.hash(utils_1.u8aConcat(index_1.deriveTicketLastKey(secrets[i]), await node.paymentChannels.utils.hash(index_1.deriveTicketKeyBlinding(secrets[i]))));
                    }
                    header.beta.set(key, parameters_1.ADDRESS_SIZE + parameters_1.MAC_SIZE + parameters_1.KEY_LENGTH);
                }
                utils_1.u8aXOR(true, header.beta, utils_1.PRG.createPRG(key, iv).digest(0, index_1.BETA_LENGTH));
            }
            header.gamma.set(index_1.createMAC(secrets[i - 1], header.beta), 0);
        }
    }
    function deriveKey(a, b) {
        return secp256k1_1.default.privateKeyTweakAdd(index_1.deriveTicketKey(a), index_1.deriveTicketKey(b));
    }
    function toString(header, secrets) {
        return peerIds.reduce((str, peerId, index) => {
            str += `\nsecret[${index}]: ${utils_1.u8aToHex(secrets[index])}\npeerId[${index}]: ${peerId.toB58String()}\npeerId[${index}] pubkey: ${utils_1.u8aToHex(peerId.pubKey.marshal())}`;
            return str;
        }, header.toString());
    }
    checkPeerIds();
