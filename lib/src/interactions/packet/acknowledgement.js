    ...
    async interact(counterparty, acknowledgement) {
        ...
        try {
            struct = await this.node.dialProtocol(counterparty, this.protocols[0]).catch(async (err) => {
                return this.node.peerRouting.findPeer(counterparty).then((peerInfo) => this.node.dialProtocol(peerInfo, this.protocols[0]));
            });
        }
        catch (err) {
...
async function handleHelper(source) {
    ...
    for await (const msg of source) {
        ...
        let record;
        const unAcknowledgedDbKey = utils_1.u8aToHex(self.node.dbKeys.UnAcknowledgedTickets(await acknowledgement.responseSigningParty, await acknowledgement.hashedKey));
        try {
