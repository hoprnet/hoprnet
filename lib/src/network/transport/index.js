...
class TCP {
    ...
    async registerDelivery(outerconnection, counterparty) {
        ...
        const timeout = setTimeout(abort.abort.bind(abort), constants_1.RELAY_CIRCUIT_TIMEOUT);
        if (!conn) {
            try {
    ...
    async dial(ma, options) {
        ...
        try {
            ...
            if (potentialRelays == null || potentialRelays.length == 0) {
                throw Error(`Destination ${chalk_1.default.yellow(ma.toString())} cannot be accessed and directly and there is no other relay node know. Connection error was:\n${err}`);
            }
    ...
    async dialWithRelay(ma, relays, options) {
        ...
        let relayConnection = await Promise.race(relays.map((relay) => new Promise(async (resolve) => {
            console.log(`[${chalk_1.default.blue(this._peerInfo.id.toB58String())}] trying to call ${chalk_1.default.yellow(ma.toString())} over relay node ${chalk_1.default.yellow(relay.id.toB58String())}`);
    ...
    async dialDirectly(ma, options) {
        console.log(`[${chalk_1.default.blue(this._peerInfo.id.toB58String())}] dailing ${chalk_1.default.yellow(ma.toString())} directly`);
