...
class TCP {
    ...
    async registerDelivery(outerconnection, counterparty) {
        ...
        if (!conn) {
            try {
                ...
                log(`[Relayer] Could not establish relayed connection to destination ${counterparty.toB58String()}. Err was:\n`, err);
                return;
            }
        }
    ...
    filter(multiaddrs) {
        ...
        return multiaddrs.filter(ma => {
            return mafmt_1.default.TCP.matches(ma.decapsulateCode(constants_1.CODE_P2P)) || mafmt_1.default.P2P.matches(ma);
        });
    }
}
exports.default = TCP;
