...
const relay_to_conn_1 = require("./relay-to-conn");
...
const RELAY_FORWARD = (from, to) => `/hopr/forward${hopr_utils_1.u8aToHex(from)}${hopr_utils_1.u8aToHex(to)}/0.0.1`;
...
class TCP {
    ...
    deliveryHandlerFactory(sender) {
        return async ({ stream }) => {
            const conn = await this._upgrader.upgradeInbound(relay_to_conn_1.relayToConn({
                stream,
                counterparty: sender,
            }));
        ...
        it_pipe_1.default(stream, (source) => {
            return async function* () {
                for await (const msg of source) {
                    ...
                    this._registrar.handle(RELAY_DELIVER(new Uint8Array([0xff])), this.deliveryHandlerFactory(sender));
                    yield OK;
                }
            }.apply(this);
        }, stream);
    }
    handleRelayUnregister({ stream }) {
        it_pipe_1.default(stream, (source) => {
            return async function* () {
                for await (const msg of source) {
                    ...
                    /* prettier-ignore */
                    new Uint8Array([0xff]), new Uint8Array([0xff])));
                    yield OK;
                }
            }.apply(this);
        });
    }
    async closeConnection(counterparty, relayer) {
        ...
        it_pipe_1.default(
        /* prettier-ignore */
        [counterparty.pubKey.marshal()], stream);
    }
            ...
            return async function* () {
                for await (const msg of source) {
                    ...
                    if (hopr_utils_1.u8aEquals(answer, OK)) {
                        this._registrar.handle(RELAY_FORWARD(
                        /* prettier-ignore */
                        new Uint8Array([0xff]), new Uint8Array([0xff])
    ...
    async dial(ma, options) {
        ...
        if (options.relay &&
            !destinationPeerId.isEqual(peer_info_1.default.isPeerInfo(options.relay) ? options.relay.id : options.relay)) {
            let relayConnection = this._registrar.getConnection(peer_info_1.default.isPeerInfo(options.relay) ? options.relay : new peer_info_1.default(options.relay));
            if (!relayConnection) {
            ...
            /* prettier-ignore */
            [destinationPeerId.pubKey.marshal()], stream, async (source) => {
                for await (const msg of source) {
                    return msg.slice();
                }
            });
            if (hopr_utils_1.u8aEquals(answer, OK)) {
                const { stream } = await relayConnection.newStream([
                    RELAY_FORWARD(new Uint8Array([0xff]), new Uint8Array([0xff])
                ...
                let conn;
                try {
                    conn = await this._upgrader.upgradeOutbound(relay_to_conn_1.relayToConn({
                        stream,
