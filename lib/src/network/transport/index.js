...
class TCP {
    ...
    async closeConnection(counterparty) {
        ...
        let conn;
        try {
            conn = await this._dialer.connectToPeer(new peer_info_1.default(this.relay));
        }
        catch (err) {
            console.log(`Could not request relayer ${this.relay.toB58String()} to tear down relayed connection. Error was:\n`, err);
            return;
        }
    ...
    async dialWithRelay(ma, options) {
        ...
        let relayConnection = this._registrar.getConnection(new peer_info_1.default(this.relay));
        if (!relayConnection) {
            relayConnection = await this._dialer.connectToPeer(new peer_info_1.default(this.relay));
        }
