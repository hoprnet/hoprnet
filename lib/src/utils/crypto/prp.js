...
function hash(data, k, iv) {
    ...
    xor_1.u8aXOR(true, data.subarray(0, HASH_LENGTH), hash.digest());
}
function encrypt(data, k, iv) {
    const cipher = crypto_1.createCipheriv(CIPHER_ALGORITHM, xor_1.u8aXOR(false, k, data.subarray(0, HASH_LENGTH)), iv);
