...
describe('test ticket generation and verification', function () {
    ...
    beforeEach(async function () {
        funder = await testing_1.getPrivKeyData(u8a.stringToU8a(configs.FUND_ACCOUNT_PRIVATE_KEY));
    ...
    it('should store ticket', async function () {
        ...
        assert_1.default(u8a.u8aEquals(await signedTicket.signer, coreConnector.self.publicKey), `Check that signer is recoverable`);
        const signedChannelCounterparty = await types_1.SignedChannel.create(coreConnector, undefined, { channel: channelType });
        assert_1.default(u8a.u8aEquals(await signedChannelCounterparty.signer, coreConnector.self.publicKey), `Check that signer is recoverable.`);
        ...
        assert_1.default(u8a.u8aEquals(signedTicket, storedSignedTicket), `Check that signedTicket is stored correctly`);
    });
    it('should store tickets, and retrieve them in a map', async function () {
        ...
        assert_1.default(u8a.u8aEquals(signedTicketA, storedSignedTicketA), `Check that signedTicketA is stored correctly`);
        const storedSignedTicketB = storedSignedTickets.get(u8a.u8aToHex(signedTicketB.ticket.challenge));
        assert_1.default(u8a.u8aEquals(signedTicketB, storedSignedTicketB), `Check that signedTicketB is stored correctly`);
    });
});
