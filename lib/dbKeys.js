...
function ChallengeKeyParse(arr) {
    return [
        arr.slice(PREFIX.length + channelSubPrefix.length, PREFIX.length + channelSubPrefix.length + constants.HASH_LENGTH),
        arr.slice(PREFIX.length + channelSubPrefix.length + constants.HASH_LENGTH + SEPERATOR.length, PREFIX.length + channelSubPrefix.length + constants.HASH_LENGTH + SEPERATOR.length + constants.HASH_LENGTH)
    ];
}
