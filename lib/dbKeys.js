...
function ChallengeKeyParse(arr) {
    ...
    const fromEnd = fromStart + types_1.Hash.SIZE;
    const toStart = fromEnd + SEPERATOR.length;
    const toEnd = toStart + types_1.Hash.SIZE;
    return [new types_1.Hash(arr.slice(fromStart, fromEnd)), new types_1.Hash(arr.slice(toStart, toEnd))];
}
...
function ChannelEntryParse(arr) {
    ...
    const fromEnd = fromStart + types_1.AccountId.SIZE;
    const toStart = fromEnd + SEPERATOR.length;
    const toEnd = toStart + types_1.AccountId.SIZE;
    return [new types_1.AccountId(arr.slice(fromStart, fromEnd)), new types_1.AccountId(arr.slice(toStart, toEnd))];
}
