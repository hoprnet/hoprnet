...
impl SnapshotValidator {
    ...
    pub async fn validate_snapshot(&self, db_path: &Path) -> SnapshotResult<SnapshotInfo> {
        ...
        let info = tokio::task::spawn_blocking(move || {
            Self::validate_sqlite_db(&db_path, &expected_tables)
        }).await??;
    ...
    pub async fn comprehensive_validation(&self, db_path: &Path) -> SnapshotResult<SnapshotInfo> {
        ...
        self.check_file_permissions(db_path).await?;
    ...
    fn validate_sqlite_db(db_path: &Path, expected_tables: &[String]) -> SnapshotResult<SnapshotInfo> {
        ...
        let latest_block: Option<i64> = conn.query_row(
            "SELECT MAX(block_number) FROM logs",
            [],
            |row| row.get(0)
    ...
    async fn check_file_permissions(&self, db_path: &Path) -> SnapshotResult<()> {
        let metadata = tokio::fs::metadata(db_path).await?;
        
        // Ensure file is readable
        if metadata.permissions().readonly() {
            warn!("Database file is read-only, this may cause issues during indexing");
        }
    ...
    pub async fn check_data_consistency(&self, db_path: &Path) -> SnapshotResult<()> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            // Check for orphaned records or other consistency issues
            // This is a placeholder - add specific consistency checks based on your schema
            
            // Example: Check that all logs have valid block numbers
            let invalid_logs: i64 = conn.query_row(
            ...
            if invalid_logs > 0 {
                return Err(SnapshotError::Validation(
                    format!("Found {} logs with invalid block numbers", invalid_logs)
                ));
            }
