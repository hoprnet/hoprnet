...
use crate::{HoprRpcOperations, NodeSafeModuleConfig, PendingTransaction};
...
impl<P: JsonRpcClient + 'static> HoprRpcOperations for RpcOperations<P> {
    ...
    // Check on-chain configuration of, node, safe, and module
                ...
                self.node_module
                    ...
                    .map_err(|e| {
                        RpcError::MulticallError(format!(
                            "Error in checking isNode function in check_node_safe_module_configuration, due to {:?}",
                            e.to_string()
                        ))
                    })?,
                false,
...
mod tests {
    ...
    #[async_std::test]
    async fn test_check_node_safe_module_configuration() -> anyhow::Result<()> {
        let _ = env_logger::builder().is_test(true).try_init();
        ...
        let result_before_including_node = rpc.check_node_safe_module_configuration((&chain_key_0).into()).await?;
        ...
        assert!(
            ...
            "safe should not the owner of a default module"
        );
        ...
        let result_with_node_included = rpc.check_node_safe_module_configuration((&chain_key_0).into()).await?;
        ...
        assert!(
            ...
            "node should not included in a default module"
        );
        assert!(
