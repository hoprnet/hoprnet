...
//!
//! This crate provides:
//! - [`traits::PathSelector`][crate::path::traits::PathSelector]: Trait for selecting multi-hop paths through the network.
//! - [`PathPlanner`]: Resolves `DestinationRouting` to `ResolvedTransportRouting`, delegating path discovery to any
//!   [`PathSelector`][crate::path::traits::PathSelector] implementation and maintaining a `moka`-backed cache of fully-validated `ValidatedPath`
//!   objects keyed by `(source, destination, options)`.
//! - [`PathPlannerConfig`][crate::path::PathPlannerConfig]: Configuration for the planner's cache and background refresh.
