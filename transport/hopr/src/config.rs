...
mod tests {
    ...
    /// A share-order sample no legal batch could reach disables the rule while looking strict.
    ///
    /// The neighbouring dial on the same rule, `max_off_front_share_fraction`, accepts `1.0` as a
    /// legitimate "tolerate everything", so this is not about refusing to let an operator switch the
    /// check off. It is about the two ways of doing it not looking alike: `1.0` states a tolerance,
    /// an unreachable sample states a threshold and delivers absence.
    ///
    /// The bound is the protocol's own ceiling, not this Exit's quota. A quota-relative bound would
    /// reject the *shipping default* sample on a small-quota Exit — where the rule genuinely is
    /// inert, but where so is `max_served_without_progress`, which gets no such check. This one
    /// therefore has no false positives: it only rejects values that could not fire at any quota.
    #[test]
    fn a_share_order_sample_no_legal_batch_could_reach_is_rejected() {
        let cfg = IncomingSessionPixConfig::default();
        validate_incoming_session_pix_config(&cfg).expect("the shipping default must validate");

        let ceiling = hopr_protocol_pix::MAX_POLYS_PER_SSA as u64
            * hopr_protocol_pix::MAX_POLY_THRESHOLD as u64
            * hopr_transport_session::MAX_SSA_BATCH_SIZE as u64;

        let unreachable = IncomingSessionPixConfig {
            supervision: SupervisorConfig {
        ...
        assert!(
            validate_incoming_session_pix_config(&unreachable).is_err(),
            "a sample no batch can reach must be refused"
        );

        // The ceiling itself is admitted, so the check cannot creep into rejecting a merely strict
        // sample: it over-counts already (widest dimensions, widest batch), and only values past even
        // that over-count are provably inert.
        let at_ceiling = IncomingSessionPixConfig {
            supervision: SupervisorConfig {
                min_share_order_sample: ceiling,
                ..cfg.supervision.clone()
            },
            ..cfg.clone()
        };
        assert!(
            validate_incoming_session_pix_config(&at_ceiling).is_ok(),
            "the ceiling itself is not provably unreachable and must be accepted"
        );

        // A small-quota Exit on the shipping sample must still start. The rule is inert there, but
        // that is a tuning question, not a reason to refuse to boot — and the cluster fixtures in
        // `hopr-lib` are configured exactly this way.
        let small_quota = IncomingSessionPixConfig {
            quota_range: 0..=100_000,
            ..cfg.clone()
        };
        validate_incoming_session_pix_config(&small_quota)
            .expect("a small accepted quota must not invalidate the default share-order sample");

        // Disabling the rule outright stays available, and stays visible as what it is.
        ...
        validate_incoming_session_pix_config(&disabled)
            .expect("a fraction of 1.0 is the supported way to disable the share-order rule");
    }
