...
/// halved relay forwarding throughput. Fair sharing under a congested pool is handled by the
/// arbiter in `hopr_utils::parallelize::cpu`, not by shrinking this default.
...
mod tests {
    ...
    fn default_pipeline_concurrency_is_cpu_scaled() {
        assert_eq!(default_pipeline_concurrency(4), 4 * PIPELINE_CONCURRENCY_PER_CPU);
        for cpus in 1..=16 {
            assert!(default_pipeline_concurrency(cpus) >= cpus);
        }
