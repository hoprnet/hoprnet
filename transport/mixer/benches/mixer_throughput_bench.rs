...
/// Fair steady-state throughput: the channel is created **once for the whole benchmark, before
/// any timing**, then reused across every size and sample. This isolates the per-message
/// send+drain cost from the one-time construction, which a per-iteration or per-sample setup
/// would otherwise fold in. See `poisson_bench.rs` for the same methodology applied to the
/// timing-wheel engine.
