...
impl TryFrom<&[u8]> for DataBytes {
    ...
    fn try_from(data: &[u8]) -> std::result::Result<Self, Self::Error> {
        if data.len() == ChainPacketComponents::SIZE {
...
impl Payload {
    /// The size of the transport payload is calculated from:
    /// 1. the ize of the HOPR shpinx packet
    /// 2. 1B to distinguish what type of payload is contained
    pub const SIZE: usize = ChainPacketComponents::SIZE + 1;
}
...
mod tests {
    ...
    fn data_bytes_cannot_be_longer_than_expected_bytes_count() {
        let too_long = [1u8; ChainPacketComponents::SIZE + 10];
    ...
    fn data_bytes_of_expected_length_can_be_built() {
        let just_right = [1u8; ChainPacketComponents::SIZE];
