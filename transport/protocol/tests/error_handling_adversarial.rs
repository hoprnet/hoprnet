...
async fn random_bytes_dropped_no_delivery() -> anyhow::Result<()> {
    let (wire_apis, apis, ticket_channels, _processes) = peer_setup_for(3).await?;
    ...
    let received = tokio::time::timeout(
        ...
        futures::StreamExt::next(&mut { apis }.into_iter().nth(2).unwrap().1),
    )
    .await;
    ...
    let ticket_result = tokio::time::timeout(
        Duration::from_millis(100),
        futures::StreamExt::next(&mut { ticket_channels }.into_iter().nth(1).unwrap()),
    )
    .await;
...
async fn corrupt_wire_frame_does_not_crash_pipeline() -> anyhow::Result<()> {
    ...
    // Inject a frame that looks like HoprPacket size but is corrupted: all zeros with one byte flipped
    let frame = vec![0u8; 500];
    let corrupted = corrupt_bytes(&frame, 42);
    inject_raw_wire(&relay_wire_tx, sender_id, corrupted);

    // Also inject completely zeroed frame
...
async fn duplicate_send_does_not_crash_pipeline() -> anyhow::Result<()> {
    // Sending the same ApplicationData payload via separate send calls exercises the replay tag
    // bloom filter; both may or may not be delivered but neither must panic.
