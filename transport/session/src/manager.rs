...
/// older SURB. A `SurbDistress` or `OutOfSurbs` signal from the counterparty overrides the estimate
/// and restores production immediately, which is what makes it safe to act on an estimate that
/// forward-path loss can inflate.
///
    ...
    S::Error: std::error::Error + Send + Sync + Clone + 'static,
{
    ...
    pub fn dispatch_message(
        ...
        if in_data.data.application_tag == HoprStartProtocol::START_PROTOCOL_MESSAGE_TAG {
            ...
            //
            // Entry side only: on an incoming Session the same signal describes our own supply
            // coming back to us, and acting on it would let a peer talk this side into minting for
            // a buffer it does not keep.
