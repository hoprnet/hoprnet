...
pub struct SessionClientConfig {
    ...
    /// counterparty is estimated to be at its target buffer size — which is what stops SURBs being
    /// delivered into a full buffer that discards them. Setting this to `true` keeps producing
    /// regardless of that estimate.
    ///
