...
///
/// The planner delegates path selection to any [`PathSelector`] implementation. The
/// default implementation is [`crate::selector::GraphPathSelector`], which uses the
    ...
    S: PathSelector + 'static,
{
    ...
    pub fn new(me: OffchainPublicKey, surb_store: Surb, resolver: R, selector: S) -> Self {
        Self {
            me,
            surb_store,
            resolver,
    ...
    ///
    /// If the node is already identified by an offchain key it is returned immediately.
    /// If the node is identified by an on-chain address, the chain resolver is queried.
    ...
    async fn resolve_path(
        ...
        let path = match options {
            ...
            RoutingOptions::Hops(hops) => {
                let hops: usize = hops.into();
                trace!(hops, "resolving path from graph-based selector");
                ...
                // Path = [intermediates..., destination] as OffchainPublicKey (source excluded).
                let path = self.selector.select_path(src_key, dest_key, hops).await?;

                // Convert to NodeId for ValidatedPath, which resolves chain addresses in the process.
                let path_as_node_ids: Vec<NodeId> = path.into_iter().map(NodeId::Offchain).collect();

                ValidatedPath::new(source, path_as_node_ids, &resolver).await?
            }
        };
    ...
    ///
    /// The returned future should be spawned as a long-lived background task.
    /// It periodically pre-warms the path cache for all reachable (destination, hops) pairs.
    ///
    /// Only available with the `runtime-tokio` feature; without it the cache is refreshed
    /// lazily on TTL expiry.
    #[cfg(feature = "runtime-tokio")]
    pub fn background_refresh(&self) -> impl std::future::Future<Output = ()> + 'static
    where
        S: BackgroundRefreshable,
    {
        self.selector.run_background_refresh()
    }
}
...
mod tests {
    ...
    use super::*;
    use crate::selector::{GraphPathSelector, PathSelectorConfig};

    // ── minimal concrete error type for tests ─────────────────────────────────
    #[derive(Debug)]
    ...
    impl TestChainApi {
        /// Build a minimal chain API with a given key → address mapping and open channels.
    ...
    fn small_config() -> PathSelectorConfig {
        PathSelectorConfig {
    ...
    async fn direct_zero_hop_path_bypasses_selector() {
        ...
        let planner = PathPlanner::new(me, surb_store, chain_api, selector);
    ...
    async fn one_hop_forward_path_uses_selector() {
        ...
        // Graph: me → dest (1-hop, but HoprCostFn for length=2 means 1 intermediate)
        // Actually for hops=1 (1 intermediate), graph length = 2, so path is me → A → dest.
        // Let's build a 1-hop chain: me → dest directly (length=1 edge).
        // For hops=1 in RoutingOptions, length = hops+1 = 2, so we need me→a→dest.
        let graph = ChannelGraph::new(me);
        graph.add_node(a);
        graph.add_node(dest);
        ...
        if let ResolvedTransportRouting::Forward { forward_path, .. } = resolved {
            // me → a → dest = 2 hops in ValidatedPath terms
            assert_eq!(
                forward_path.num_hops(),
                2,
