...
- CPU-bound: `hopr_types::parallelize::cpu::spawn_blocking()` (rayon pool)
...
- **Auth**: Two methods - `X-Auth-Token` header, `Bearer` token
...
- NEVER log passwords or raw keys
- Scrypt params: Production uses higher `n` than tests (default n=2 is testing only)

### Cryptographic Operations

- All crypto uses `hopr_types::parallelize::cpu::spawn_blocking()` to avoid blocking tokio
- Example: `OffchainPublicKey::from_peerid()` is blocking - wrap appropriately or use the helper `peer_id_to_public_key()` which uses a cache
...
2. `tokio::task::spawn_blocking` for CPU → use `hopr_types::parallelize::cpu::spawn_blocking`
...
9. Exposing internal errors in API → map to user-friendly messages
10. Hardcoding config values → use env vars or config files with defaults
11. Missing back-pressure on channels → implement back-pressure handling and timeouts
