...
impl SealedHost {
    ...
    pub fn seal(host: IpOrHost, peer_id: PeerId) -> crate::errors::Result<Self> {
        ...
        if host_str.len() < Self::MAX_LEN_WITH_PADDING {
            let pad_len = hopr_api::types::crypto_random::random_integer(0, (Self::MAX_LEN_WITH_PADDING as u64).into());
            for _ in 0..pad_len {
                host_str.push(Self::PADDING_CHAR);
            }
        }

        hopr_api::types::crypto::seal::seal_data(host_str.as_bytes(), peer_id)
            .map(Self::Sealed)
    ...
    pub fn unseal(self, key: &hopr_api::types::crypto::keypairs::OffchainKeypair) -> crate::errors::Result<IpOrHost> {
        match self {
            SealedHost::Plain(host) => Ok(host),
            SealedHost::Sealed(enc) => hopr_api::types::crypto::seal::unseal_data(&enc, key)
                .map_err(|e| NetworkTypeError::Other(e.to_string()))
                .and_then(|data| {
