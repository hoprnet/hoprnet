...
impl CopyBuffer {
        ...
        R: AsyncRead + ?Sized,
    {
        ...
        let res = reader.poll_read(cx, &mut this.buf[this.cap..]);
        if let Poll::Ready(Ok(read)) = res {
            let filled_len = read;
            this.read_done = this.cap == filled_len;
            this.cap = filled_len;
        }
        res.map(|r| r.map(|_| ()))
    }
...
mod tests {
    ...
    // These tests intentionally use tokio only for the convenience of tokio::io::DuplexStream
    ...
    async fn test_client_to_server() -> anyhow::Result<()> {
        let (mut client_tx, client_rx) = tokio::io::duplex(8); // Create a mock duplex stream
        let (server_rx, mut server_tx) = tokio::io::duplex(32); // Create a mock duplex stream
        ...
        let result = crate::utils::copy_duplex(&mut client_rx.compat(), &mut server_rx.compat(), 2, 2).await?;
    ...
    async fn test_server_to_client() -> anyhow::Result<()> {
        let (mut client_tx, client_rx) = tokio::io::duplex(32); // Create a mock duplex stream
        let (server_rx, mut server_tx) = tokio::io::duplex(8); // Create a mock duplex stream
