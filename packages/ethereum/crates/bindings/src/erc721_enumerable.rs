...
pub mod erc721_enumerable {
    ...
    impl<M> Clone for ERC721Enumerable<M> {
        fn clone(&self) -> Self {
            ERC721Enumerable(self.0.clone())
        }
    }
    ...
    impl<M> std::fmt::Debug for ERC721Enumerable<M> {
    ...
    impl<M: ethers::providers::Middleware> ERC721Enumerable<M> {
        ...
        pub fn new<T: Into<ethers::core::types::Address>>(
            ...
            ethers::contract::Contract::new(address.into(), ERC721ENUMERABLE_ABI.clone(), client)
                .into()
        }
        ...
        pub fn events(&self) -> ethers::contract::builders::Event<M, ERC721EnumerableEvents> {
            self.0.event_with_filter(Default::default())
        }
    }
    impl<M: ethers::providers::Middleware> From<ethers::contract::Contract<M>> for ERC721Enumerable<M> {
    ...
    impl ::std::fmt::Display for ERC721EnumerableEvents {
        fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
            match self {
                ERC721EnumerableEvents::ApprovalFilter(element) => element.fmt(f),
                ERC721EnumerableEvents::ApprovalForAllFilter(element) => element.fmt(f),
                ERC721EnumerableEvents::TransferFilter(element) => element.fmt(f),
            }
        }
    }
    ...
    impl ethers::core::abi::AbiDecode for ERC721EnumerableCalls {
        fn decode(
            ...
            {
                return Ok(ERC721EnumerableCalls::Approve(decoded));
            }
            if let Ok(decoded) =
                <BalanceOfCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(ERC721EnumerableCalls::BalanceOf(decoded));
            }
            if let Ok(decoded) =
                <GetApprovedCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(ERC721EnumerableCalls::GetApproved(decoded));
            }
            if let Ok(decoded) =
                <IsApprovedForAllCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(ERC721EnumerableCalls::IsApprovedForAll(decoded));
            }
            if let Ok(decoded) = <NameCall as ethers::core::abi::AbiDecode>::decode(data.as_ref()) {
                return Ok(ERC721EnumerableCalls::Name(decoded));
            }
            if let Ok(decoded) =
                <OwnerOfCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(ERC721EnumerableCalls::OwnerOf(decoded));
            }
            if let Ok(decoded) =
                <SafeTransferFromCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(ERC721EnumerableCalls::SafeTransferFrom(decoded));
            }
            if let Ok(decoded) =
            ...
            {
                return Ok(ERC721EnumerableCalls::SafeTransferFromWithData(decoded));
            }
            if let Ok(decoded) =
                <SetApprovalForAllCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(ERC721EnumerableCalls::SetApprovalForAll(decoded));
            }
            if let Ok(decoded) =
                <SupportsInterfaceCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(ERC721EnumerableCalls::SupportsInterface(decoded));
            }
            if let Ok(decoded) = <SymbolCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(ERC721EnumerableCalls::Symbol(decoded));
            }
            if let Ok(decoded) =
            ...
            {
                return Ok(ERC721EnumerableCalls::TokenURI(decoded));
            }
            if let Ok(decoded) =
            ...
            {
                return Ok(ERC721EnumerableCalls::TransferFrom(decoded));
            }
    ...
    impl ethers::core::abi::AbiEncode for ERC721EnumerableCalls {
        fn encode(self) -> Vec<u8> {
            match self {
                ERC721EnumerableCalls::Approve(element) => element.encode(),
                ERC721EnumerableCalls::BalanceOf(element) => element.encode(),
                ERC721EnumerableCalls::GetApproved(element) => element.encode(),
                ERC721EnumerableCalls::IsApprovedForAll(element) => element.encode(),
                ERC721EnumerableCalls::Name(element) => element.encode(),
                ERC721EnumerableCalls::OwnerOf(element) => element.encode(),
                ERC721EnumerableCalls::SafeTransferFrom(element) => element.encode(),
                ERC721EnumerableCalls::SafeTransferFromWithData(element) => element.encode(),
                ERC721EnumerableCalls::SetApprovalForAll(element) => element.encode(),
                ERC721EnumerableCalls::SupportsInterface(element) => element.encode(),
                ERC721EnumerableCalls::Symbol(element) => element.encode(),
                ...
                ERC721EnumerableCalls::TokenURI(element) => element.encode(),
                ERC721EnumerableCalls::TotalSupply(element) => element.encode(),
                ERC721EnumerableCalls::TransferFrom(element) => element.encode(),
            }
        }
    }
    impl ::std::fmt::Display for ERC721EnumerableCalls {
        fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
            match self {
                ERC721EnumerableCalls::Approve(element) => element.fmt(f),
                ERC721EnumerableCalls::BalanceOf(element) => element.fmt(f),
                ERC721EnumerableCalls::GetApproved(element) => element.fmt(f),
                ERC721EnumerableCalls::IsApprovedForAll(element) => element.fmt(f),
                ERC721EnumerableCalls::Name(element) => element.fmt(f),
                ERC721EnumerableCalls::OwnerOf(element) => element.fmt(f),
                ERC721EnumerableCalls::SafeTransferFrom(element) => element.fmt(f),
                ERC721EnumerableCalls::SafeTransferFromWithData(element) => element.fmt(f),
                ERC721EnumerableCalls::SetApprovalForAll(element) => element.fmt(f),
                ERC721EnumerableCalls::SupportsInterface(element) => element.fmt(f),
                ERC721EnumerableCalls::Symbol(element) => element.fmt(f),
                ERC721EnumerableCalls::TokenByIndex(element) => element.fmt(f),
                ERC721EnumerableCalls::TokenOfOwnerByIndex(element) => element.fmt(f),
                ERC721EnumerableCalls::TokenURI(element) => element.fmt(f),
                ERC721EnumerableCalls::TotalSupply(element) => element.fmt(f),
                ERC721EnumerableCalls::TransferFrom(element) => element.fmt(f),
            }
        }
    }
    impl ::std::convert::From<ApproveCall> for ERC721EnumerableCalls {
        fn from(var: ApproveCall) -> Self {
            ERC721EnumerableCalls::Approve(var)
        }
    }
    impl ::std::convert::From<BalanceOfCall> for ERC721EnumerableCalls {
        fn from(var: BalanceOfCall) -> Self {
            ERC721EnumerableCalls::BalanceOf(var)
        }
    }
    impl ::std::convert::From<GetApprovedCall> for ERC721EnumerableCalls {
        fn from(var: GetApprovedCall) -> Self {
            ERC721EnumerableCalls::GetApproved(var)
        }
    }
    impl ::std::convert::From<IsApprovedForAllCall> for ERC721EnumerableCalls {
        fn from(var: IsApprovedForAllCall) -> Self {
            ERC721EnumerableCalls::IsApprovedForAll(var)
        }
    }
    impl ::std::convert::From<NameCall> for ERC721EnumerableCalls {
        fn from(var: NameCall) -> Self {
            ERC721EnumerableCalls::Name(var)
        }
    }
    impl ::std::convert::From<OwnerOfCall> for ERC721EnumerableCalls {
        fn from(var: OwnerOfCall) -> Self {
            ERC721EnumerableCalls::OwnerOf(var)
        }
    }
    impl ::std::convert::From<SafeTransferFromCall> for ERC721EnumerableCalls {
        fn from(var: SafeTransferFromCall) -> Self {
            ERC721EnumerableCalls::SafeTransferFrom(var)
        }
    }
    impl ::std::convert::From<SafeTransferFromWithDataCall> for ERC721EnumerableCalls {
        fn from(var: SafeTransferFromWithDataCall) -> Self {
            ERC721EnumerableCalls::SafeTransferFromWithData(var)
        }
    }
    impl ::std::convert::From<SetApprovalForAllCall> for ERC721EnumerableCalls {
        fn from(var: SetApprovalForAllCall) -> Self {
            ERC721EnumerableCalls::SetApprovalForAll(var)
        }
    }
    impl ::std::convert::From<SupportsInterfaceCall> for ERC721EnumerableCalls {
        fn from(var: SupportsInterfaceCall) -> Self {
            ERC721EnumerableCalls::SupportsInterface(var)
        }
    }
    impl ::std::convert::From<SymbolCall> for ERC721EnumerableCalls {
        fn from(var: SymbolCall) -> Self {
            ERC721EnumerableCalls::Symbol(var)
        }
    }
    ...
    impl ::std::convert::From<TokenURICall> for ERC721EnumerableCalls {
        fn from(var: TokenURICall) -> Self {
            ERC721EnumerableCalls::TokenURI(var)
        }
    }
    ...
    impl ::std::convert::From<TransferFromCall> for ERC721EnumerableCalls {
        fn from(var: TransferFromCall) -> Self {
            ERC721EnumerableCalls::TransferFrom(var)
        }
    }
