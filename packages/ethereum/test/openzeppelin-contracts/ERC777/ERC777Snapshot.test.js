...
const ERC777SnapshotMock = artifacts.require('ERC777SnapshotMock')

const { expect } = require('chai')

contract('ERC777Snapshot', function (accounts) {
  ...
  beforeEach(async function () {
    this.erc1820 = await singletons.ERC1820Registry(initialHolder)
    this.token = await ERC777SnapshotMock.new(name, symbol, initialHolder, initialSupply)
  })
  ...
  describe('totalSupplyAt', function () {
    it('reverts with a snapshot id of 0', async function () {
      await expectRevert(this.token.totalSupplyAt(0), 'ERC777Snapshot: id is 0')
    })

    it('reverts with a not-yet-created snapshot id', async function () {
      await expectRevert(this.token.totalSupplyAt(1), 'ERC777Snapshot: nonexistent id')
    })
    ...
    context('with initial snapshot', function () {
      ...
      context('with supply changes after the snapshot', function () {
        beforeEach(async function () {
          await this.token.mint(other, new BN('50'), '0x00', '0x00')
  ...
  describe('balanceOfAt', function () {
    it('reverts with a snapshot id of 0', async function () {
      await expectRevert(this.token.balanceOfAt(other, 0), 'ERC777Snapshot: id is 0')
    })

    it('reverts with a not-yet-created snapshot id', async function () {
      await expectRevert(this.token.balanceOfAt(other, 1), 'ERC777Snapshot: nonexistent id')
    })
