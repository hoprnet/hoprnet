...
describe('Accounts', function () {
  it('should initialize account', async function () {
    ...
    await expect(accounts.initializeAccountInternal(ACCOUNT_A.address, ACCOUNT_A.uncompressedPublicKey, SECRET_2))
  ...
  it("should update account's secret", async function () {
    ...
    await expect(accounts.updateAccountSecretInternal(ACCOUNT_A.address, SECRET_1))
      .to.emit(accounts, 'AccountSecretUpdated')
      .withArgs(ACCOUNT_A.address, SECRET_1, '2')
  ...
  it("should fail to update account's secret when secret is empty", async function () {
    ...
    // give empty SECRET
    await expect(accounts.updateAccountSecretInternal(ACCOUNT_A.address, ethers.constants.HashZero)).to.be.revertedWith(
  ...
  it("should fail to update account's secret when secret is the same as before", async function () {
    ...
    // give same SECRET
    await expect(accounts.updateAccountSecretInternal(ACCOUNT_A.address, SECRET_1)).to.be.revertedWith(
