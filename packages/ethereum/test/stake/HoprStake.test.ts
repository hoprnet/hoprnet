...
import { advanceTimeForNextBlock, deployContractFromFactory, latestBlock, latestBlockTime } from '../utils';
...
(whitehatTestOnly ? describe.skip : describe)('HoprStake', function () {
    ...
    const reset = async () => {
        ...
        erc777 = await deployContractFromFactory(deployer, "ERC777Mock", [adminAddress, utils.parseUnits('5000000', 'ether'), "ERC777Mock", "M777", []]);
        
        // create NFT and stake contract
        nftContract = await deployContractFromFactory(deployer, "HoprBoost", [adminAddress, ""]);
        stakeContract = await deployContractFromFactory(deployer, "HoprStake", [nftContract.address, adminAddress, erc677.address, erc777.address]);
        stake2Contract = await deployContractFromFactory(deployer, "HoprStake2", [nftContract.address, adminAddress, erc677.address, erc777.address]);
    ...
    describe('integration tests', function () {
        ...
        describe('LOCK_TOKEN and other ERC677 token', () => {
            ...
            it('cannot receive random 677 with `transferAndCall()`', async () => {
               ...
               await expect(randomERC677.connect(participants[2]).transferAndCall(stakeContract.address, constants.One, '0x'))
            ...
            it('can receive LOCK_TOKEN with `transferAndCall()`', async () => {
                ...
                // tx = await erc677.connect(participants[0]).transferAndCall(stakeContract.address, utils.parseUnits('1000', 'ether'), '0x'); 
                // stake 1000 LOCK_TOKEN
                await expect(erc677.connect(participants[0]).transferAndCall(stakeContract.address, utils.parseUnits('1000', 'ether'), '0x'))
        ...
        describe('REWARD_TOKEN and other ERC777 token', () => {
                ...
                it('cannot receive random 777 with `send()`', async () => {
                    ...
                    randomERC777 = await deployContractFromFactory(deployer, "ERC777Mock", [participantAddresses[2], utils.parseUnits('5000000', 'ether'), "ERC777Mock", "M777", []]);

                    // randomERC777 = await deployContractFromFactory(deployer, "ERC777Mock");
                    // await randomERC777.mintInternal(participantAddresses[2], utils.parseUnits('5000000', 'ether'), '0x', '0x'); 
                    await expect(randomERC777.connect(participants[2]).send(stakeContract.address, constants.One, '0x')).to.be.revertedWith("HoprStake: Sender must be wxHOPR token");
                });
                ...
                it('cannot receive REWARD_TOKEN from a random account', async () => {
                    ...
                    await expect(erc777.connect(participants[2]).send(stakeContract.address, constants.One, '0x')).to.be.revertedWith("HoprStake: Only accept owner to provide rewards");
                }); 
                ...
                it('can receive REWARD_TOKEN with `send()`', async () => {
                    ...
                    // tx = await erc777.connect(admin).send(stakeContract.address, constants.One, '0x'); 
        ...
        describe('nftBoost and other ERC721 tokens', function () {
            ...
            it ('cannot receive an boost-like random ERC721 token', async () => {
                ...
                await expect(randomERC721.connect(participants[0]).functions["safeTransferFrom(address,address,uint256)"](participantAddresses[0], stakeContract.address, 0)).to.be.revertedWith("HoprStake: Cannot SafeTransferFrom tokens other than HoprBoost.");
            });
            it ('cannot redeem a boost when the deadline has passed', async () => {
                ...
                await expect(nftContract.connect(participants[2]).functions["safeTransferFrom(address,address,uint256)"](participantAddresses[2], stakeContract.address, 6)).to.be.revertedWith("HoprStake: Cannot redeem an expired boost.");
            });
            it ('cannot reclaim a HOPRBoost', async () => {
                await expect(stake2Contract.connect(admin).reclaimErc721Tokens(nftContract.address, 0)).to.be.revertedWith("HoprStake: Cannot claim HoprBoost NFT");
            });
        ...
        describe('Before program starts', function () {
            ...
            it('can redeem HODLr token', async function () {
                await expect(nftContract.connect(participants[0]).functions["safeTransferFrom(address,address,uint256)"](participantAddresses[0], stakeContract.address, 0))
                    .to.emit(stakeContract, 'Redeemed')
                    .withArgs(participantAddresses[0], constants.Zero.toString(), constants.One.toString());
            });
            ...
            it ('has nothing to claim', async () => {
                await expect(stakeContract.claimRewards(participantAddresses[0])).to.be.revertedWith('HoprStake: Nothing to claim');
            });
        ...
        describe('At BASIC_START', function () {
            ...
            it ('has insufficient pool', async () => {
                await expect(stakeContract.claimRewards(participantAddresses[0])).to.be.revertedWith('HoprStake: Insufficient reward pool.');
            });
        });
        ...
        describe('During the staking program v1 / before staking v2', function () {
            describe('Staking v1', function () {
                it ('advance 2 blocks (10 seconds), there is more rewards to be claimed.', async () => {
                    ...
                    const lastBlock = await latestBlock(hre.ethers.provider)

                    const duration = 2;
                    await advanceTimeForNextBlock(hre.ethers.provider, lastBlock + duration); // advance two blocks - 2 seconds
                    const currentAccount = await stakeContract.accounts(participantAddresses[0]);
                ...
                it('can redeem another (less good) HODLr token', async function () {
                    ...
                    // redeem NFT #7
                    await expect(nftContract.connect(participants[0]).functions["safeTransferFrom(address,address,uint256)"](participantAddresses[0], stakeContract.address, 7))
                        .to.emit(stakeContract, 'Redeemed')
                        .withArgs(participantAddresses[0], '7', constants.Zero.toString());
                ...
                it('can redeem another (better) HODLr token', async function () {
                    ...
                    await expect(nftContract.connect(participants[0]).functions["safeTransferFrom(address,address,uint256)"](participantAddresses[0], stakeContract.address, 2))
                        .to.emit(stakeContract, 'Redeemed')
                        .withArgs(participantAddresses[0], '2', constants.One.toString());
                ...
                it('can redeem another HODLr token of another category', async function () {
                    ...
                    await expect(nftContract.connect(participants[0]).functions["safeTransferFrom(address,address,uint256)"](participantAddresses[0], stakeContract.address, 2))
                        .to.emit(stakeContract, 'Redeemed')
                        .withArgs(participantAddresses[0], '8', constants.One.toString());
                ...
                it ('cannot lock tokens when length does not match', async () => {
                    await expect(stakeContract.connect(admin).lock([participantAddresses[0]], ['1', '2'])).to.be.revertedWith('HoprStake: Length does not match');
                });

                it ('can lock tokens', async () => {
                    await expect(stakeContract.connect(admin).lock([participantAddresses[2]], [utils.parseUnits('1.0', 'ether').toString()]))
                        .to.emit(stakeContract, 'Staked')
                ...
                it ('cannot unlock tokens', async () => {
                    await expect(stakeContract.unlock(participantAddresses[0])).to.be.revertedWith('HoprStake: Program is ongoing, cannot unlock stake.');
                });
            ...
            describe('Staking v2', function () {
                ...
                it('can redeem HODLr token', async function () {
                    ...
                    await expect(nftContract.connect(participants[0]).functions["safeTransferFrom(address,address,uint256)"](participantAddresses[0], stakeContract.address, 3))
                        .to.emit(stakeContract, 'Redeemed')
                        .withArgs(participantAddresses[0], '3', constants.One.toString());
                });
                ...
                it ('has nothing to claim', async () => {
                    await expect(stake2Contract.claimRewards(participantAddresses[0])).to.be.revertedWith('HoprStake: Nothing to claim');
                });
        ...
        describe('After v1 PROGRAM_END', function () {
            ...
            describe('Before unlock', function () {
                ...
                it('cannot receive random 677 with `transferAndCall()`', async () => {
                    // bubbled up
                    await expect(erc677.connect(participants[1]).transferAndCall(stakeContract.address, constants.One, '0x')).to.be.revertedWith('ERC677Mock: failed when calling onTokenTransfer');
                }); 
                it('cannot redeem NFT`', async () => {
                    ...
                    await expect(nftContract.connect(participants[1]).functions["safeTransferFrom(address,address,uint256)"](participantAddresses[1], stakeContract.address, 9)).to.be.revertedWith('HoprStake: Program ended, cannot redeem boosts.');
                }); 
                it('cannot lock', async () => {
                    await expect(stakeContract.connect(admin).lock([participantAddresses[0]], ['1'])).to.be.revertedWith('HoprStake: Program ended, cannot stake anymore.');
                }); 
            }); 
            describe('Unlock', function () {
                ...
                before('can unlock - reclaim rewards and receives original tokens', async () => {
                    ...
                    await expect(stakeContract.connect(participants[1]).unlock(participantAddresses[1]))
                        ...
                        .withArgs(participantAddresses[1], utils.parseUnits('1.0', 'ether').toString());
                }); 
            ...
            describe('Staking v2', function () {
                ...
                it ('has insufficient pool', async () => {
                    await expect(stake2Contract.claimRewards(participantAddresses[0])).to.be.revertedWith('HoprStake: Insufficient reward pool.');
                });
            });
        });
        ...
        describe('During the staking v2 program', function () {
            it('can redeem another (less good) HODLr token', async function () {
                ...
                await expect(nftContract.connect(participants[0]).functions["safeTransferFrom(address,address,uint256)"](participantAddresses[0], stake2Contract.address, 10))
                        .to.emit(stake2Contract, 'Redeemed')
                        .withArgs(participantAddresses[0], '10', constants.One.toString());
                // const receipt = await hre.ethers.provider.waitForTransaction(tx.hash);
                // const from = await getParamFromTxResponse(
                //     receipt, stake2Contract.interface.getEvent("Redeemed").format(), 1, stake2Contract.address.toLowerCase(), "Redeem the token"
                // );
            ...
            it('can redeem another (better) HODLr token', async function () {
                ...
                await expect(nftContract.connect(participants[0]).functions["safeTransferFrom(address,address,uint256)"](participantAddresses[0], stake2Contract.address, 5))
                        .to.emit(stake2Contract, 'Redeemed')
                        .withArgs(participantAddresses[0], '5', constants.One.toString());
            ...
            it ('claims rewards', async () => {
                ...
                await expect(stake2Contract.claimRewards(participantAddresses[0]))
                        .to.emit(stake2Contract, 'Claimed')
            ...
            it('can redeem another HODLr token of another category', async function () {
                ...
                // redeem NFT #11
                await expect(nftContract.connect(participants[0]).functions["safeTransferFrom(address,address,uint256)"](participantAddresses[0], stake2Contract.address, 11))
                        .to.emit(stake2Contract, 'Redeemed')
                        .withArgs(participantAddresses[0], '11', constants.One.toString());
            ...
            it ('cannot unlockFor tokens', async () => {
                await expect(stake2Contract.unlockFor(participantAddresses[0])).to.be.revertedWith('HoprStake: Program is ongoing, cannot unlock stake.');
            });
