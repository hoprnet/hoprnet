...
describe('HoprWhitehat', function () {
    ...
    const reset = async () => {
        ...
        stakeContract = await deployContractFromFactory(deployer, "HoprStake", [nftContract.address, adminAddress, xHopr.address, wxHopr.address]);

        // create whitehat contract
        whitehatContract = await deployContractFromFactory(deployer, "HoprWhitehat", [adminAddress, nftContract.address, stakeContract.address, xHopr.address, wxHopr.address])
        ...
        describe('REWARD_TOKEN and other ERC777 token', () => {
            ...
            describe('global switch is on', () => {
                ...
                it('can not receive random 777 with `send()` when the globalSwitch is on', async () => {
                    await expect(randomERC777.connect(stakers[0]).send(whitehatContract.address, constants.One, '0x')).to.be.revertedWith(
                        'can only be called from wxHOPR'
                    )
                }); 
        ...
        describe('After v1 PROGRAM_S1_END', function () {
            ...
            deadLocked.map((lockedIndex) => {
                it(`cannnot unlock for deadlocked staker ${lockedIndex}`, async function () {
                    await expect(stakeContract.connect(stakers[lockedIndex]).unlock(stakerAddresses[lockedIndex])).to.be.revertedWith(
