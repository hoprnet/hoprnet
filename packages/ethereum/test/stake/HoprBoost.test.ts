    ...
    describe('integration tests', function () {
        ...
        describe('mint', function () {
            it('allows admin - a minter - to mint', async function () {
                await expect(nftContract.connect(admin).mint(goldHodlerAddresses[0], BADGES[0].type, BADGES[0].rank, BADGES[0].nominator, BADGES[0].deadline))
        ...
        describe('baseURI', function () {
            it('fails when non admin sets URI', async function () {
                await expect(
                    nftContract.connect(deployer).updateBaseURI(baseURI)
                ).to.be.revertedWith(`AccessControl: account ${deployerAddress.toLowerCase()} is missing role ${constants.HashZero}`)
            });
