...
describe('HoprStakeSeason3', function () {
    let deployer: Signer;
    let admin: Signer;
    ...
    const reset = async () => {
        ...
        stakeContract = await deployContractFromFactory(deployer, "HoprStakeSeason3", [nftContract.address, adminAddress, erc677.address, erc777.address]);
    ...
    describe('After PROGRAM_S3_END', function () {
        ...
        it('succeeds in advancing block to PROGRAM_S3_END + 1', async function () {
            await advanceTimeForNextBlock(hre.ethers.provider, PROGRAM_S3_END + 1);
        ...
        it('cannot redeem NFT', async () => {
            ...
            await expect(nftContract.connect(participants[1]).functions["safeTransferFrom(address,address,uint256)"](participantAddresses[1], stakeContract.address, 1)).to.be.revertedWith(
