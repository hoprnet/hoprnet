...
describe('Registry proxy for stake v2', () => {
      ...
      const cannotSelfRegister = [1, 3, 4]
        ...
        canSelfRegister.forEach((accountIndex) => {
            it(`participant ${accountIndex} is still whitelisted`, async () => {
                expect(await hoprStakingProxyForNetworkRegistry.isRequirementFulfilled(participantAddresses[accountIndex])).to.be.true;
            })
        })
        cannotSelfRegister.forEach((accountIndex) => {
            it(`participant ${accountIndex} is not whitelisted`, async () => {
                expect(await hoprStakingProxyForNetworkRegistry.isRequirementFulfilled(participantAddresses[accountIndex])).to.be.false;
            })
        })
    })
    ...
    describe(`Owner batch-add NFTs`, () => {
      beforeEach(async () => {
        await hoprStakingProxyForNetworkRegistry.connect(owner).ownerBatchAddNftTypeAndRank([NFT_TYPE[0], NFT_TYPE[1]], [NFT_RANK[1], NFT_RANK[0]])
      })

      it('fails to when array length does not match', async () => {
        await expect(
            hoprStakingProxyForNetworkRegistry.connect(owner).ownerBatchAddNftTypeAndRank([NFT_TYPE[0]], [NFT_RANK[1], NFT_RANK[0]])
    ...
    describe(`Owner batch-remove NFTs`, () => {
      beforeEach(async () => {
        await hoprStakingProxyForNetworkRegistry.connect(owner).ownerBatchRemoveNftTypeAndRank([NFT_TYPE[0], NFT_TYPE[1]], [NFT_RANK[1], NFT_RANK[0]])
      })

      it('fails to when array length does not match', async () => {
        await expect(
            hoprStakingProxyForNetworkRegistry.connect(owner).ownerBatchRemoveNftTypeAndRank([NFT_TYPE[0]], [NFT_RANK[1], NFT_RANK[0]])
