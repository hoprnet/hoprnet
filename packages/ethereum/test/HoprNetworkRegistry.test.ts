...
describe('HoprNetworkRegistry', () => {
    ...
    const reset = async () => {
        ...
        // mock staking contract
        stakeV2Fake = await createFakeStakeV2Contract(participantAddresses);
        // deploy network registry
        const contract = await ethers.getContractFactory("HoprNetworkRegistry");
        const artifact = await contract.connect(deployer).deploy(stakeV2Fake.address, ownerAddress, INITIAL_MIN_STAKE);
        hoprNetworkRegistry = await artifact.deployed();
    }
    ...
    describe('Owner can update important parameters of the contract', () => {
      ...
      it('fail to batch-add NFTs existing {type: 0, rank: 1} and {type: 1, rank: 1} when length does not match', async () => {
        await expect(hoprNetworkRegistry.connect(owner)
        .ownerBatchAddNftTypeAndRank([NFT_TYPE[0], NFT_RANK[1]], [1]))
        .to.be.revertedWith(
          'HoprNetworkRegistry: ownerBatchAddNftTypeAndRank lengths mismatch'
        )
      });
      it('owner to batch-add NFTs existing {type: 0, rank: 1} and {type: 1, rank: 1}', async () => {
        await expect(hoprNetworkRegistry.connect(owner).ownerBatchAddNftTypeAndRank([NFT_TYPE[0], NFT_TYPE[1]], [NFT_RANK[1], NFT_RANK[1]]))
          .to.emit(hoprNetworkRegistry, 'AddedNftTypeAndRank')
      ...
      it('owner to batch-remove NFTs existing {type: 0, rank: 1} and {type: 1, rank: 1}', async () => {
        await expect(hoprNetworkRegistry.connect(owner).ownerBatchRemoveNftTypeAndRank([NFT_TYPE[0], NFT_TYPE[1]], [NFT_RANK[1], NFT_RANK[1]]))
          .to.emit(hoprNetworkRegistry, 'RemovedNftTypeAndRank')
          .withArgs(NFT_TYPE[1], NFT_RANK[1])
      });
      it('fail to let owner to batch-remove when nftTypes and nftRanks have different length', async () => {
        await expect(hoprNetworkRegistry.connect(owner)
        .ownerBatchRemoveNftTypeAndRank([NFT_TYPE[0], NFT_TYPE[1]], [NFT_RANK[1]]))
        .to.be.revertedWith(
          'HoprNetworkRegistry: ownerRemoveNftTypeAndRank lengths mismatch'
        )
      });
      it('owner to whitelist addresses with amount < threshold and >= threshold', async () => {
          await expect(hoprNetworkRegistry.connect(owner).ownerAddToWhitelist([hoprAddress(0), hoprAddress(1)], participantAddresses.slice(0, 2), [300, 1800]))
          .to.emit(hoprNetworkRegistry, 'OwnerAddedToWhitelist').withArgs(participantAddresses[0], 300, hoprAddress(0))
          .to.emit(hoprNetworkRegistry, 'OwnerAddedToWhitelist').withArgs(participantAddresses[1], 1800, hoprAddress(1))
      });
      it('fail to let owner to whitelist when hoprAddresses and stakers have different length', async () => {
        ...
        .to.be.revertedWith(
          'HoprNetworkRegistry: hoprAddresses and stakers lengths mismatch'
        )
      });
      it('fail to let owner to whitelist when amounts and stakers have different length', async () => {
        ...
        .to.be.revertedWith(
          'HoprNetworkRegistry: amounts and stakers lengths mismatch'
        )
      });
      it('fail to let owner to whitelist when an amount is zero', async () => {
        ...
        .to.be.revertedWith(
          'HoprNetworkRegistry: staked amount should be above zero'
        )
      });
      ...
      it('owner to batch-remove from whitelist, provided a long list but only two get removed', async () => {
        ...
        .to.emit(hoprNetworkRegistry, 'OwnerRemovedFromWhitelist').withArgs(participantAddresses[0])
        .to.emit(hoprNetworkRegistry, 'OwnerRemovedFromWhitelist').withArgs(participantAddresses[1])
    });
    });
    ...
    describe('Self whitelist', () => {
      ...
      it('fail to whitelist staker with stake of low threshold and eligible NFT', async () => {
        await expect(hoprNetworkRegistry.connect(participants[2]).addToWhitelist(hoprAddress(2)))
        .to.be.revertedWith(
      ...
      it('fail to whitelist staker with stake of low threshold and non-eligible NFT', async () => {
        await expect(hoprNetworkRegistry.connect(participants[3]).addToWhitelist(hoprAddress(3)))
        .to.be.revertedWith(
          'HoprNetworkRegistry: staked amount does not meet the threshold'
        )
      });
      it('can be whitelisted by the owner', async () => {
        await expect(hoprNetworkRegistry.connect(owner).ownerAddToWhitelist([hoprAddress(1), hoprAddress(2), hoprAddress(3)], participantAddresses.slice(1, 4), [2000, 100, 100]))
        .to.emit(hoprNetworkRegistry, 'OwnerAddedToWhitelist').withArgs(participantAddresses[1], 2000, hoprAddress(1))
        .to.emit(hoprNetworkRegistry, 'OwnerAddedToWhitelist').withArgs(participantAddresses[2], 100, hoprAddress(2))
        .to.emit(hoprNetworkRegistry, 'OwnerAddedToWhitelist').withArgs(participantAddresses[3], 100, hoprAddress(3))
      });
    });
    describe('Integration test', () => {
      beforeEach(async () => {
          ...
          await hoprNetworkRegistry.connect(owner).ownerAddToWhitelist([hoprAddress(1), hoprAddress(2), hoprAddress(3)], participantAddresses.slice(1, 4), [2000, 100, 100])
      });
      ...
      describe('Lower threshold to 100', () => {
        ...
        canSelfRegister.forEach((accountIndex) => {
          it(`whitelist staker ${accountIndex} with stake of high threshold and eligible NFT`, async () => {
            await expect(hoprNetworkRegistry.connect(participants[accountIndex]).addToWhitelist(hoprAddress(accountIndex)))
              .to.emit(hoprNetworkRegistry, 'AddedToWhitelist')
