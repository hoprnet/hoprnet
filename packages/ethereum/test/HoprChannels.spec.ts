...
describe('HoprChannels', async function () {
  ...
  const {
    ...
    generateChannelId,
    ChannelStatus
  } = await import('@hoprnet/hopr-utils')
  ...
  const redeemArgs = (ticket: InstanceType<typeof AcknowledgedTicket>): Parameters<HoprChannels['redeemTicket']> => [
  ...
  const createTicket = async (
    ...
    TicketValues & {
      nextCommitment: string
      counterparty: string
      ticket: InstanceType<typeof AcknowledgedTicket>
    }
  > => {
   ...
   */
  const ACCOUNT_AB_CHANNEL_ID = generateChannelId(
    Address.fromString(ACCOUNT_A.address),
    Address.fromString(ACCOUNT_B.address)
  ).toHex()
  const ACCOUNT_BA_CHANNEL_ID = generateChannelId(
    Address.fromString(ACCOUNT_B.address),
    Address.fromString(ACCOUNT_A.address)
  ).toHex()

  const PROOF_OF_RELAY_SECRET_0 = ethers.utils.solidityKeccak256(['string'], ['PROOF_OF_RELAY_SECRET_0'])
  // const PROOF_OF_RELAY_SECRET_1 = ethers.utils.solidityKeccak256(['string'], ['PROOF_OF_RELAY_SECRET_1'])

  const SECRET_0 = ethers.utils.solidityKeccak256(['string'], ['secret'])
  const SECRET_1 = ethers.utils.solidityKeccak256(['bytes32'], [SECRET_0])
  const SECRET_2 = ethers.utils.solidityKeccak256(['bytes32'], [SECRET_1])

  const WIN_PROB_100 = percentToUint256(100)
  const WIN_PROB_0 = percentToUint256(0)
  ...
  const recoverPublicKeyFromSigner = async (
    ...
    return PublicKey.fromSignatureString(hash, txReceipt.r as string, txReceipt.s as string, txReceipt.v as number)
  }
  ...
  describe('test internals with mock', function () {
    ...
    it('should get channel id', async function () {
      expect(await channels.getChannelIdInternal(ACCOUNT_A.address, ACCOUNT_B.address)).to.be.equal(
