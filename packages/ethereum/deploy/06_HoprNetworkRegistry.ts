...
const main = async function (hre: HardhatRuntimeEnvironment) {
    ...
    const adminAddress = network.name == 'hardhat' ? deployer.address : environmentConfig['network_registry_admin_address']
    ...
    await deployments.deploy('HoprNetworkRegistry', {
        ...
        args: [
            environmentConfig['stake_v2_contract_address'],
            adminAddress,
            minStakes[deploymentType]
        ]
    })
}
