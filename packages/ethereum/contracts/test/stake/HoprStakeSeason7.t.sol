...
import '../../src/stake/HoprStakeSeason7.sol';
...
contract HoprStakeSeason7Test is Test, ERC1820RegistryFixtureTest, PermittableTokenFixtureTest {
  ...
  function test_batchStakeFor(uint256 accLength) public {
    ...
    (bool successReadTokenBalance, bytes memory returndataTokenBalance) = LOCK_TOKEN_ADDRESS.staticcall(
    ...
    vm.store(
      ...
      bytes32(
        stdstore
          .target(address(LOCK_TOKEN_ADDRESS))
    ...
    (bool successReadBalanceOfStake, bytes memory returndatBalanceOfStake) = LOCK_TOKEN_ADDRESS.staticcall(
      abi.encodeWithSignature('balanceOf(address)', address(hoprStakeSeason7))
    );
    ...
    (bool successReadBalanceOfOwner, bytes memory returndatBalanceOfOwner) = LOCK_TOKEN_ADDRESS.staticcall(
      abi.encodeWithSignature('balanceOf(address)', OWNER)
    );
