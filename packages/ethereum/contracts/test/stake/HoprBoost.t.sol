...
contract HoprBoostTest is Test {
  ...
  function test_ReclaimERC20Tokens() public {
    ...
    vm.mockCall(erc20Contract, abi.encodeWithSignature('balanceOf(address)', address(hoprBoost)), abi.encode(1 ether));
    vm.mockCall(erc20Contract, abi.encodeWithSignature('transfer(address,uint256)', admin, 1 ether), abi.encode(true));
    vm.prank(admin);
