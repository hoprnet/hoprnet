...
contract HoprWrapperTest is Test, ERC1820RegistryFixtureTest {
  ...
  function test_RecoverTokens(uint256 amount) public {
    ...
    uint256 balanceSlot = stdstore
      .target(address(xHoprToken))
      .sig('balanceOf(address)')
      .with_key(address(hoprWrapper))
      .find();
    ...
    vm.expectCall(
      address(xHoprToken),
      abi.encodeWithSignature('transfer(address,uint256)', newOwner, INTIAL_BALANCE - amount)
    );
