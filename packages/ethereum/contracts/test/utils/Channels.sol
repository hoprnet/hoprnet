...
contract ChannelsUtilsTest is Test {
    ...
    function assertEqChannels(HoprChannels.Channel memory channel1, HoprChannels.Channel memory channel2) public returns (bool) {
    ...
    function getChannelFromTuple(HoprChannels hoprChannels, bytes32 channelId) public returns (HoprChannels.Channel memory) {
        (uint256 a,bytes32 b, uint256 c, uint256 d, HoprChannels.ChannelStatus e, uint256 f, uint32 g) = hoprChannels.channels(channelId);
    ...
    function getChannelHash(HoprChannels.Channel memory channel) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(channel.balance, channel.commitment, channel.ticketEpoch, channel.ticketIndex, channel.status, channel.channelEpoch, channel.closureTime));
    }
}
