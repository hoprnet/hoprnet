...
library BoostUtilsLib {
     ...
     */
    function getBoostTypeAtIndex(address hoprBoostAddr, uint256 index) public returns (bool hasIndex, string memory boostType) {
        HoprBoost hoprBoost = HoprBoost(hoprBoostAddr);

        // Option 1: use contract type
        try hoprBoost.typeAt(index) returns (string memory boostType) {
            return (true, boostType);
