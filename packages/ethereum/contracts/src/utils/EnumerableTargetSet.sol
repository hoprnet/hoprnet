...
enum Permission { NONE, BLOCK_ALL, SPECIFIC_FALLBACK_BLOCK, SPECIFIC_FALLBACK_ALLOW, ALLOW_ALL}
 ...
 * (Permission) as uint8: defaultTargetPermission                                       (for the target)
 * (Permission) as uint8: defaultRedeemTicketSafeFunctionPermisson                      (for Channels contract)
 * (Permission) as uint8: defaultBatchRedeemTicketsSafeFunctionPermisson                (for Channels contract)
 * (Permission) as uint8: defaultCloseIncomingChannelSafeFunctionPermisson              (for Channels contract)
 * (Permission) as uint8: defaultInitiateOutgoingChannelClosureSafeFunctionPermisson    (for Channels contract)
 * (Permission) as uint8: defaultFinalizeOutgoingChannelClosureSafeFunctionPermisson    (for Channels contract)
 * (Permission) as uint8: defaultFundChannelMultiFunctionPermisson                      (for Channels contract)
 * (Permission) as uint8: defaultSetCommitmentSafeFunctionPermisson                     (for Channels contract)
 * (Permission) as uint8: defaultApproveFunctionPermisson                               (for Token contract)
 * (Permission) as uint8: defaultSendFunctionPermisson                                  (for Token contract)
 */
...
library TargetUtils {
    ...
    function getDefaultTargetPermission(Target target) internal pure returns (Permission) {
        // left shift 160 + 8 + 8 bits then right shift 256 - 8 bits
        return Permission(uint8((Target.unwrap(target) << 176) >> 248));
    }

    function getDefaultFunctionPermissionAt(Target target, uint256 position) internal pure returns (Permission) {
        // left shift 160 + 8 + 8 + 8 + 8 * pos bits then right shift 256 - 8 bits
        uint256 leftShiftBy = 184 + 8 * position;
        return Permission(uint8((Target.unwrap(target) << leftShiftBy) >> 248));
    }
