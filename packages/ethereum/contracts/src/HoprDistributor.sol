...
contract HoprDistributor is Ownable {
  ...
  function getSchedule(string calldata name) external view returns (uint128[] memory, uint128[] memory) {
    return (schedules[name].durations, schedules[name].percents);
  }
   ...
   */
  function revokeAccount(address account, string calldata scheduleName) external onlyOwner {
   ...
   */
  function _getClaimable(Schedule storage schedule, Allocation storage allocation) internal view returns (uint128) {
