...
abstract contract ERC777Snapshot is ERC777 {
   ...
   */
  function _valueAt(Snapshot[] storage snapshots, uint128 _block) internal view returns (uint256) {
  ...
  function updateValueAtNow(Snapshot[] storage snapshots, uint256 _value) internal {
    ...
    if ((lenSnapshots == 0) || (snapshots[lenSnapshots - 1].fromBlock < block.number)) {
      snapshots.push(Snapshot(uint128(block.number), uint128(_value)));
    } else {
