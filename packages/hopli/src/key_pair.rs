...
pub fn read_identities(files: Vec<PathBuf>, password: &String) -> Result<HashMap<String, NodeIdentity>, HelperErrors> {
    ...
    for file in files.iter() {
        ...
        match HoprKeys::read_eth_keystore(file_str, password) {
            Ok((keys, needs_migration)) => {
                ...
                results.insert(String::from(file_key.to_str().unwrap()), NodeIdentity::new(keys));
            }
            Err(e) => {
