...
export function getPeers(
  ...
  let peers = node.network.peerStore.peers.map((peer) =>
    PeerId.createFromB58String(peer.id)
  );
 ...
 */
export function getMyOpenChannelInstances(
  node: Hopr<HoprCoreConnector>
 ...
 * @returns a promise that resolves to an array of peer ids
 */
export async function getMyOpenChannels(
  node: Hopr<HoprCoreConnector>
): Promise<PeerId[]> {
 ...
 * @returns a promise that resolves to an array of peer ids
 */
export async function getPartyOpenChannels(
  node: Hopr<HoprCoreConnector>,
  party: PeerId
): Promise<PeerId[]> {
  ...
  const channelAccountIds = channels.map((channel) => {
    return u8aEquals(channel.partyA, partyPubKey)
      ? channel.partyB
      : channel.partyA;
  });
