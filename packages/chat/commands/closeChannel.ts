...
export default class CloseChannel extends AbstractCommand {
  ...
  async execute(query?: string): Promise<string | void> {
    if (query == null) {
      return chalk.red(
        `Invalid arguments. Expected 'close <peerId>'. Received '${query}'`
      );
    }
    ...
    try {
      const channel = await this.node.paymentChannels.channel.create(
        ...
        async (counterparty: Uint8Array) =>
          this.node.interactions.payments.onChainKey.interact(
            await pubKeyToPeerId(counterparty)
          )
      );
      ...
      unsubscribe();
      if (isPending) {
        return `${chalk.green(`Successfully closed channel`)} ${chalk.yellow(
          u8aToHex(await channel.channelId)
        )}.`;
      } else {
        return `${chalk.yellow(`Initiated channel for closure`)} ${chalk.yellow(
          u8aToHex(await channel.channelId)
        )}.`;
      }
    } catch (err) {
  ...
  async autocomplete(query: string, line: string): Promise<AutoCompleteResult> {
    ...
    try {
      peerIdStrings = await this.node.paymentChannels.channel.getAll(
        async (channel: ChannelInstance) =>
          (
            await pubKeyToPeerId(await channel.offChainCounterparty)
          ).toB58String(),
    ...
    const hits = query
      ? peerIdStrings.filter((peerId: string) => peerId.startsWith(query))
      : peerIdStrings;
    return [
      hits.length ? hits.map((str: string) => `close ${str}`) : ["close"],
      line,
    ];
  }
}
