...
export default class OpenChannel extends AbstractCommand {
  ...
  async execute(query?: string): Promise<string | void> {
    if (query == null || query == "") {
      return chalk.red(
        `Invalid arguments. Expected 'open <peerId>'. Received '${query}'`
      );
    }
    ...
    const exitQuestion = `Do you want to cancel (${chalk.green(
      "Y"
    )} / ${chalk.red("n")}) : `;

    do {
      tmpFunds = await new Promise<string>((resolve) =>
        this.rl.question(tokenQuestion, resolve)
      );
      funds = new BigNumber(tmpFunds);
      clearString(tokenQuestion + tmpFunds, this.rl);

      if (tmpFunds.length == 0) {
        let decision = await new Promise<string>((resolve) =>
          this.rl.question(exitQuestion, resolve)
        );
      ...
      funds
        .times(
          new BigNumber(10).pow(
            this.node.paymentChannels.types.Balance.DECIMALS
          )
        )
        .toString()
    );
    ...
    const unsubscribe = startDelayedInterval(
    ...
    try {
      ...
      await this.node.paymentChannels.channel.create(
        counterPartyPubKey,
        async () =>
          this.node.interactions.payments.onChainKey.interact(counterparty),
        channelBalance,
      ...
      unsubscribe();
      return `${chalk.green(`Successfully opened channel`)} ${chalk.yellow(
        u8aToHex(channelId)
      )}`;
    } catch (err) {
      unsubscribe();
  ...
  async autocomplete(query: string, line: string): Promise<AutoCompleteResult> {
    const peersWithOpenChannel = await getOpenChannels(
      this.node,
    ...
    const hits = query
      ? peers.filter((peerId: string) => peerId.startsWith(query))
      : peers;

    return [
      hits.length ? hits.map((str: string) => `open ${str}`) : ["open"],
      line,
    ];
  }
}
