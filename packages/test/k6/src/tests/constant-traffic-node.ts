...
import { AccountApi, ChannelsApi, MesssagesApi, NodeApi, Addresses, NodeBalance, SendMessageRequest, TOKEN_HOPR_MIN, TOKEN_NATIVE_MIN } from "../api/index";
...
export class ConstantTrafficNode {
    ...
    public constructor(node: HoprNode, dataPool?: {addresses: {[key:string]: Addresses}, peers: {[key:string]: Peer[]}}) {
        this.node = node;
    ...
    private checkConnectivity(){
        ...
        if(!check(nodeInfo, { 'green node connectivity': (ni) => ni.connectivityStatus === this.node.thresholds.connectivityStatus.valueOf()})){
    ...
    private checkChannels(){
        ...
        if(!check(channelsResponse.incoming, { 'Not enough incomming channels opened': (ic) => ic.length >= this.node.thresholds.incommingOpenChannels})){
            fail(`Node '${this.node.alias}' does not have enough incomming channels opened`);
        }
        if(!check(channelsResponse.outgoing, { 'Not enough outgoing channels opened': (oc) => oc.length >= this.node.thresholds.outgoingOpenChannels})){
    ...
    private getPeers(peerType: string): Peer[] {
        ...
        return peerType === 'connected' ?  JSON.parse(peersResponse.body).connected : JSON.parse(peersResponse.body).announced
