pub mod wasm {
    ...
    /// Turns a JsValue stream into a Vec<u8> stream
    ///
    /// ```
    /// use futures::stream::{Stream,StreamExt};
    /// use js_sys::Uint8Array;
    /// use utils_misc::async_iterable::wasm::to_vec_u8_stream;
    /// use wasm_bindgen::prelude::*;
    ///
    /// pub async fn take_stream(stream: impl Stream<Item = Result<JsValue, JsValue>>) -> Vec<Result<Vec<u8>,String>> {
    ///   stream.map(to_vec_u8_stream).collect::<Vec<Result<Vec<u8>,String>>>().await
    /// }
    /// ```
    ...
    /// use wasm_bindgen::prelude::*;
    ///
    /// pub async fn take_stream(mut stream: impl Stream<Item = Result<Vec<u8>,String>> + Unpin) -> Result<JsValue, JsValue> {
    ///    to_jsvalue_stream(stream.next().await)
    /// }
    /// ```
    pub fn to_jsvalue_stream(item: Option<Result<Vec<u8>, String>>) -> Result<JsValue, JsValue> {
        match item {
    ...
    ///    stream: Box<dyn Stream<Item = Result<Vec<u8>, String>> + Unpin>, // must not be pub
    /// }
    /// #[wasm_bindgen]
    ...
    ///
    ///     let stream = stream.map(to_vec_u8_stream);
    ///
