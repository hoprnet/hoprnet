...
describe('Rust - TS serialization/deserialization tests', async function () {
  ...
  it('channel id', async function () {
    let id1 = generateChannelId(
      TsPublicKey.deserialize(pub_key_1.serialize(false)).toAddress(),
      TsPublicKey.deserialize(pub_key_2.serialize(false)).toAddress()
    )
    let id2 = generate_channel_id(
      PublicKey.from_privkey(private_key_1).to_address(),
      PublicKey.from_privkey(private_key_2).to_address()
    )
  ...
  it('channel entry', async function () {
    ...
    assert(
      ChannelEntry.deserialize(ts_channel_entry.serialize()).eq(rs_channel_entry),
      'channel entry serde test failed'
    )
  })
