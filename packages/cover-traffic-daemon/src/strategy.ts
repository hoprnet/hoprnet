    ...
    if (this.data.openChannelCount() > CT_INTERMEDIATE_HOPS + 1) {
      for (let openChannel of state.ctChannels) {
        ...
        if (channel && channel.status == ChannelStatus.Open) {
          ...
          log(`Unknown error in sending traffic. Channel is ${channel.status}; openChannel is ${JSON.stringify({...openChannel, destination: openChannel.destination.toB58String()})}`)
        }
      }
    } else {
