...
const unreleasedTokenInputs: UnreleasedTokens[] = [
  ...
  {...unreleasedTokenInput, link: {}},
  {
    ...
    link: {
      ...
      "16Uiu2HAmBCcc822eURPRu6YXuSNmPZn2tJ1nEePNPUsz8xRNZRV7": [
        "0x1d216b8706be76f7906eb5872835ce5567fd2ef5"
      ]
    }
  }
]
    ...
    it('should validate allocation entries', function () {
      for (const nodeEthAddress of Object.keys(unreleasedTokens.allocation)) {
        ...
        for (let index = 0; index < allocations.length; index++) {
          ...
          assert(new BN(allocation.lowerBlock).ltn(allocation.upperBlock), `invalid unreleased tokens: ${allocation.unreleased}`)
          if (index > 0) {
            assert(new BN(allocation.lowerBlock).gten(allocations[index - 1].upperBlock), `overlapping schedule: ${allocation.unreleased}`)
          }
        }
      }
    })
