...
export async function statsReducer(this: Coverbot, instructionWrapper: Instruction, message: IMessage) {
  ...
  switch (instructionWrapper.subcommand) {
    case StatsSubCommands.connected: {
      ...
      log(`- statsReducer | ${BotCommands.stats} command :: ${StatsSubCommands.connected} subcommand retrieving state from snapshot with value ${state}`)
      const connectedNodes: number = state && state.connectedNodes ? state.connectedNodes : 0
      log(`- statsReducer | ${BotCommands.stats} command :: ${StatsSubCommands.connected} subcommand retrieving connected nodes from state with value ${connectedNodes}`)
