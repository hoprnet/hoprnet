...
impl From<&wasm::OutgoingChannelStatus> for OutgoingChannelStatus {
    fn from(x: &wasm::OutgoingChannelStatus) -> Self {
        OutgoingChannelStatus {
            address: Address::from_str(&x.address).expect("invalid Ethereum address given"),
...
pub mod wasm {
            ...
            Ok(StrategyTickResult {
                w: super::StrategyTickResult::new(
                    ...
                    to_close
                        .iter()
                        .map(|s| {
                            Address::from_str(String::from(s).as_str())
    ...
    macro_rules! strategy_tick {
        ($strategy:ident, $balance:ident, $addresses:ident, $outgoing_channels:ident, $quality_of:ident) => {
            Ok(StrategyTickResult {
                w: $strategy.tick(
                    $balance,
                    $addresses.into_iter().map(|v| {
                        v.and_then(|v| {
                            v.as_string()
