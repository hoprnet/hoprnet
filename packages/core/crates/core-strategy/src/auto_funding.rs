...
pub struct AutoFundingStrategyConfig {
    ...
    pub min_stake_treshold: Balance,
}

impl Default for AutoFundingStrategyConfig {
    fn default() -> Self {
        Self {
            min_stake_treshold: Balance::zero(BalanceType::HOPR)
        }
    }
}
...
impl<Db: HoprCoreEthereumDbActions> SingularStrategy for AutoFundingStrategy<Db> {
    async fn on_channel_state_changed(&self, channel: &ChannelEntry) -> crate::errors::Result<()> {
        if channel.balance.lt(&self.cfg.min_stake_treshold) {
            info!("{self} strategy: stake on {channel} is below treshhold {} < {}", channel.balance, self.cfg.min_stake_treshold);
            let to_stake = self.cfg.min_stake_treshold.sub(&channel.balance);
            if let Err(e) = fund_channel(self.db.clone(), self.tx_sender.clone(), channel.get_id(), to_stake.clone()).await {
                error!("{self} strategy: failed to issue staking of {channel} with {to_stake}");
            } else {
