...
export async function createTicket(
  ...
  log(
    `balances ${channel.balance.toString()} - ${outstandingTicketBalance.toString()} = ${balance.toString()} should >= ${amount.toString()} in channel open to ${!channel.destination ? "" : channel.destination.toB58String()}`
  )
