...
describe('unit test heartbeat', async () => {
  ...
  beforeEach(async () => {
    ...
    heartbeat = new Heartbeat(peers, subscribe, send, hangUp, { network: 'xdai', address: '0x0' })
  })
  ...
  it('test heartbeat flow', async () => {
    let generateMock = (i: string | number) => {
      ...
      let heartbeat = new Heartbeat(peers, subscribe, send, hangUp, { network: 'xdai', address: '0x0' })
