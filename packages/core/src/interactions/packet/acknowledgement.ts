...
export class AcknowledgementInteraction {
  ...
  async handleAcknowledgement(msg: Uint8Array, remotePeer: PeerId): Promise<void> {
    ...
    if (!acknowledgement.validate(PublicKey.from_peerid_str(this.privKey.toString()), PublicKey.from_peerid_str(remotePeer.toString()))) {
    ...
    const ack = new AcknowledgedTicket(unacknowledged.ticket, response.clone(), PREIMAGE_PLACE_HOLDER.clone(), unacknowledged.signer)
