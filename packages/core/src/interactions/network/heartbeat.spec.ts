...
describe('check heartbeat mechanism', function () {
  async function generateNode(options?: { timeoutIntentionally?: boolean }) {
    ...
    node.multiaddrs.add(Multiaddr('/ip4/0.0.0.0/tcp/0'))
    ...
    await Promise.all([
      new Promise((resolve) => {
        Bob.network.heartbeat.once('beat', (peerId: PeerId) => {
          assert(peerId.isEqual(Alice.peerId), 'connection must come from Alice')
          resolve()
        })
      }),
      Alice.interactions.network.heartbeat.interact(Bob.peerId)
    ])
  ...
  it('should trigger a heartbeat timeout', async function () {
    ...
    let before = Date.now()
    try {
      await Alice.interactions.network.heartbeat.interact(Bob.peerId)
    } catch (err) {
      errorThrown = true
    }
