...
describe('POST /messages', function () {
  ...
  it('should not work when parameters are incorrect', async function () {
    ...
    const res1 = await request(service).post(`/api/v3/messages`).send({ tag, body, recipient, hops: 0 })
    expect(res1.status).to.equal(400)

    const res2 = await request(service).post(`/api/v3/messages`).send({ tag, body, recipient, hops: 4 })
    expect(res2.status).to.equal(400)

    const res3 = await request(service).post(`/api/v3/messages`).send({ tag: 70000, body, recipient, hops })
    expect(res3.status).to.equal(400)

    const res4 = await request(service).post(`/api/v3/messages`).send({ tag: -1, body, recipient, hops })
    expect(res4.status).to.equal(400)

    const res5 = await request(service).post(`/api/v3/messages`).send({ tag, body, recipient: 'hello peer id', hops })
