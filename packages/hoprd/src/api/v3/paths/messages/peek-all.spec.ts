...
describe('GET /messages/peek-all', function () {
  let node: Hopr
  let service: any
  ...
  it('should return nothing when inbox is empty', async function () {
    ...
    const res = await request(service).post(`/api/v3/messages/peek-all`).send({ tag })
  ...
  it('should return nothing when other inbox is not empty', async function () {
    ...
    const res = await request(service).post(`/api/v3/messages/peek-all`).send({ tag: otherTag })
    expect(res.status).to.equal(200)
    expect(res.body.messages.length).to.equal(0)
    expect(await inbox.size(tag)).to.equal(1)
