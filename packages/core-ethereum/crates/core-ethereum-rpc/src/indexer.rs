            ...
            if current >= latest {
                ...
                loop {
                    match get_block_with_logs_from_provider(current, &filter, provider.clone()).await {
                        ...
                        Err(NoSuchBlock) => {
                            let next_wait = current_backoff.max(Duration::from_millis(100));
                            debug!("no block #{current}, waiting {} ms", next_wait.as_millis());

                            sleep(next_wait + Duration::from_millis(50)).await;
                            current_backoff = next_wait / 2;
                        }
                        Err(e) => {
